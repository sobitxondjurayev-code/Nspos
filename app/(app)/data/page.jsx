...
import {
  Upload, FileSpreadsheet, Trash2, ChevronRight, CircleCheck, TriangleAlert, Zap,
} from "lucide-react";
import {
...
export default function DataUpload() {
  ...
  return (
    <div>
      ...
      {/* Katalog, qoldiq, mijoz va cheklar endi API orqali o'zi keladi
          (Sozlamalar → Billz'dan yangilash). Bu sahifa o'chirilmadi:
          Billz'ning ayrim hisobotlari (P&L, samaradorlik, hisobdan
          chiqarish) API'da yo'q va faqat Excel bilan keladi. */}
      <div className="card p-5 mb-7 flex items-start gap-3 border-brand/40">
        <Zap size={20} className="text-brand shrink-0 mt-0.5" />
        <p className="font-semibold">
          {t("Tovar, qoldiq, mijoz va cheklar endi Billz'dan o'zi tortiladi — qo'lda yuklash shart emas.")}{" "}
          <Link href="/settings" className="text-brand hover:underline">{t("Sozlamalar → Billz'dan yangilash")}</Link>
          {". "}
          {t("Bu sahifa API bermaydigan hisobotlar uchun qoladi (P&L, samaradorlik, hisobdan chiqarish).")}
        </p>
      </div>
