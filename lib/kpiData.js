...
// 2026-09-03: rahbar Sozlamalar → "Biznes qoidalari"da o'zgartiradi
// (`companies.sozlamalar.kpi.payDays`); kodda turgani STANDART.
const PAY_DAYS_STANDART = [1, 5, 10, 15, 20, 25];
export const payDays = () => sozlama("kpi.payDays", PAY_DAYS_STANDART);
...
// jadval o'zi 1/5/10/15/20/25 ga qaytadi. Endi ro'yxat Sozlamalarda
// ("Shu oyga cheklovni ochish" tugmasi) — kodda "2026-08" standart.
const PAY_ANY_DAY_STANDART = ["2026-08"];
export const payAnyDayMonths = () => sozlama("kpi.payAnyDayMonths", PAY_ANY_DAY_STANDART);
export const isPayAnyDayMonth = (month) => payAnyDayMonths().includes(String(month ?? "").slice(0, 7));

export const isPayDay = (dayOfMonth, month = null) =>
  isPayAnyDayMonth(month) || payDays().includes(Number(dayOfMonth));
...
export const KPI_TYPES = {
  b2b: {
    ...
    columns: [
      ...
      { key: "diff", label: "Savdo − tushum", kind: "calc", calc: "diff" },
  ...
  b2c_retention: {
    ...
    columns: [
      ...
      { key: "cash", label: "Naqd ($)", kind: "money" },
      { key: "payme", label: "Payme ($)", kind: "money" },
      // Servis ustuni do'kon menejerinikidek. Ilgari bu yerda YO'Q edi
      // va o'sha paytda zarari ham yo'q edi — raqamni menejer qo'lda
      // yozardi, retention menejerda esa do'kon yo'q. 2026-08-26 dan
      // servis Billz'dan avtomat keladi: ustun bo'lmasa, montaj puli
      // "Jami tushum" ichida KO'RINMASDAN turardi (retention menejer
      // montajli do'konga biriktirilgan kuni).
      { key: "service", label: "Servis ($)", kind: "money" },
      { key: "income", label: "Jami tushum", kind: "calc", calc: "income" },
      { key: "sales", label: "Savdo ($)", kind: "money" },
      { key: "diff", label: "Savdo − tushum", kind: "calc", calc: "diff" },
...
// ── STANDART QOIDALAR va SOZLAMADAN USTIGA YOZISH (2026-09-03) ──
// Kodda turgani — standart. Rahbar Sozlamalar → "KPI qoidalari"da
// o'zgartirsa `companies.sozlamalar.kpi.rules.<tur>` ga yoziladi va
// `typeOf()` uni standart USTIGA qo'yadi (bo'limlar — revision, late… —
// ichma-ich birlashadi; salesTiers ro'yxati butunicha almashadi).
// Reja qatoriga (`kpi_plan.data.rules`) saqlangan qoida esa shundan
// ham ustun — lekin faqat STANDARTDAN FARQ QILGAN qismi (`tozaRules`):
// ilgari `savePlan` butun qoidani muhrlab qo'yardi va sozlama o'zgarsa
// eski oydan meros bo'lib kelaverardi.
export const KPI_RULES_STANDART = Object.fromEntries(
  Object.entries(KPI_TYPES).map(([k, v]) => [k, v.defaultRules]));
const bolimmi = (v) => v && typeof v === "object" && !Array.isArray(v);
function rulesFor(t) {
  const std = KPI_RULES_STANDART[t] ?? KPI_RULES_STANDART[DEFAULT_TYPE];
  const s = sozlama(`kpi.rules.${t}`, null);
  if (!bolimmi(s)) return std;
  const out = { ...std };
  for (const [k, v] of Object.entries(s)) {
    if (!(k in std)) continue;                      // noma'lum kalit — e'tiborsiz
    out[k] = bolimmi(v) && bolimmi(std[k]) ? { ...std[k], ...v } : v;
  }
  return out;
}
// Reja qatoridagi qoidadan faqat standartdan farq qilgani qoladi
export function tozaRules(r, t) {
  if (!bolimmi(r)) return {};
  const std = KPI_RULES_STANDART[t] ?? KPI_RULES_STANDART[DEFAULT_TYPE];
  const out = {};
  for (const [k, v] of Object.entries(r)) {
    if (JSON.stringify(v) !== JSON.stringify(std[k])) out[k] = v;
  }
  return out;
}
export const typeOf = (t) => {
  const kalit = t in KPI_TYPES ? t : DEFAULT_TYPE;
  return { ...KPI_TYPES[kalit], defaultRules: rulesFor(kalit) };
};
...
import { registerModule, upsert, remove as dbRemove, DEMO_MODE, xotiraYangilandi } from "./db";
...
import { kunlikKirimStore } from "./kassaIncome";
import { sozlama, getUsdRate } from "./companyData";
...
// ── BAZADA NIMA BOR — uchala jadval uchun ham ──────────────────
// Ilgari faqat kunlar kuzatilardi (`seenDayId`), reja va tur esa yo'q.
// Ikki oqibati bor edi (2026-08-24 da jurnal bo'yicha topildi):
//
//   1. Har sahifa ochilganda `syncUp()` bazadan ENDIGINA kelgan 17 ta
//      rejani va 16 ta turni qaytarib yozardi. 22-avgustda shu 5 530 +
//      5 184 ta ortiqcha POST bergan.
//   2. Xavflisi: `bootstrap()` uchala jadvalni `Promise.all` bilan
//      o'qiydi, ya'ni TARTIB KAFOLATLANMAGAN. `kpi_day` birinchi
//      tugasa, `syncUp()` hali bazadan kelmagan — localStorage'dagi
//      ESKI rejani bazaga yozib yuborardi. Boshqa menejer o'zgartirgan
//      reja shu bilan jimgina orqaga qaytardi.
//
// Endi: har jadval bazadan kelganini o'zi belgilaydi, `syncUp()` esa
// UCHALASI ham kelgandan keyin va faqat BAZADA YO'Q yozuvni yuboradi.
const seenDayId = new Set();          // bazada bor kunlar
const seenPlanId = new Set();         // bazada bor rejalar
const seenAssign = new Map();         // staffId -> bazadagi kpi_type
const keldi = new Set();              // qaysi jadval bazadan keldi
let syncUpBajarildi = false;

// Kunlik yozuvni ko'p bosishda har harfda emas, tinchlangach yozamiz —
// yozayotganda qayta chizilib fokus yo'qolmasin.
// ── BAZA RAD ETSA XOTIRA VA localStorage ORQAGA QAYTADI (2026-09-03) ──
// Ilgari uchala yozuv `silent` edi: RLS rad etsa (usta/kassir boshqa
// xodimning kunini yozsa, sessiya tugasa) toast chiqmasdi, raqam esa
// localStorage'da qolib, F5 dan keyin ham "saqlangan" ko'rinardi —
// rahbar kompyuterida esa yo'q edi. Endi baza tasdiqlagan oxirgi holat
// eslab qolinadi (`tasdiq*`) va rad bo'lsa o'shanga qaytiladi, ekran
// qayta chiziladi, toast sababini aytadi.
const tasdiqKun = new Map();          // id → baza tasdiqlagan kun
const tasdiqReja = new Map();         // id → baza tasdiqlagan reja
const tasdiqTur = new Map();          // staffId → baza tasdiqlagan tur
const rad = (r) => r && !r.ok;

function orqagaKun(id) {
  const t = tasdiqKun.get(id);
  days = t ? days.map((d) => (d.id === id ? t : d)) : days.filter((d) => d.id !== id);
  seenDayId.delete(id);
  persist(); xotiraYangilandi();
}
function orqagaReja(id) {
  const t = tasdiqReja.get(id);
  plans = t ? plans.map((p) => (p.id === id ? t : p)) : plans.filter((p) => p.id !== id);
  seenPlanId.delete(id);
  persist(); xotiraYangilandi();
}
function orqagaTur(staffId) {
  const t = tasdiqTur.get(staffId);
  const a = { ...assignments };
  if (t) a[staffId] = t; else delete a[staffId];
  assignments = a;
  seenAssign.delete(staffId);
  persist(); xotiraYangilandi();
}
...
function pushDay(d) {
  ...
  seenDayId.add(d.id);
  return upsert("kpi_day", { id: d.id, staff_id: d.staffId, date: d.date, data: dayData(d) }, undefined, "KPI kun")
    .then((r) => { if (r?.ok) tasdiqKun.set(d.id, d); else if (rad(r)) orqagaKun(d.id); return r; });
}
...
function pushPlan(p) {
  ...
  seenPlanId.add(p.id);
  return upsert("kpi_plan", { id: p.id, staff_id: p.staffId, month: p.month, data: planData(p) }, undefined, "KPI reja")
    .then((r) => { if (r?.ok) tasdiqReja.set(p.id, p); else if (rad(r)) orqagaReja(p.id); return r; });
}
function pushAssign(staffId, kpiType) {
  if (DEMO_MODE) return;
  seenAssign.set(staffId, kpiType);
  return upsert("kpi_assign", { staff_id: staffId, kpi_type: kpiType }, undefined, "KPI tur")
    .then((r) => { if (r?.ok) tasdiqTur.set(staffId, kpiType); else if (rad(r)) orqagaTur(staffId); return r; });
}

// Bazadan kelmagan (faqat shu qurilmada bor) yozuvlarni yuqoriga surish.
//
// Uchala jadval ham bazadan kelgunicha KUTILADI — yuqoridagi izohga
// qarang: erta yuborilgan reja boshqa qurilmadagi yangisini bosib
// ketardi. Bir marta bajariladi.
const KPI_JADVALLAR = ["kpi_day", "kpi_plan", "kpi_assign"];
function syncUp() {
  if (DEMO_MODE || syncUpBajarildi) return;
  if (!KPI_JADVALLAR.every((t) => keldi.has(t))) return;
  syncUpBajarildi = true;
  for (const d of days) if (!seenDayId.has(d.id)) pushDay(d);
  for (const p of plans) if (!seenPlanId.has(p.id)) pushPlan(p);
  for (const [sid, ty] of Object.entries(assignments)) {
    if (seenAssign.get(sid) !== ty) pushAssign(sid, ty);
  }
}
...
registerModule("kpi_assign", {
  ...
  restore: (rows) => {
    ...
    for (const r of rows) {   // baza ustun
      next[r.staff_id] = r.kpi_type; seenAssign.set(r.staff_id, r.kpi_type); tasdiqTur.set(r.staff_id, r.kpi_type);
    }
    assignments = next; persist();
    keldi.add("kpi_assign"); syncUp();
  },
  apply: ({ eventType, new: row, old }) => {
    if (eventType === "DELETE") { const a = { ...assignments }; delete a[old.staff_id]; assignments = a; tasdiqTur.delete(old.staff_id); }
    else { assignments = { ...assignments, [row.staff_id]: row.kpi_type }; tasdiqTur.set(row.staff_id, row.kpi_type); }
    persist();
  },
});
...
registerModule("kpi_plan", {
  ...
  restore: (rows) => {
    ...
    for (const r of rows) { byId.set(r.id, r); seenPlanId.add(r.id); tasdiqReja.set(r.id, r); }   // baza ustun
    plans = [...byId.values()]; persist();
    keldi.add("kpi_plan"); syncUp();
  },
  apply: ({ eventType, new: row, old }) => {
    if (eventType === "DELETE") { plans = plans.filter((p) => p.id !== old.id); tasdiqReja.delete(old.id); }
    else { const it = { ...row.data, id: row.id, staffId: row.staff_id, month: row.month };
      plans = [...plans.filter((p) => p.id !== it.id), it]; tasdiqReja.set(it.id, it); }
    persist();
  },
});
...
registerModule("kpi_day", {
  ...
  restore: (rows) => {
    ...
    for (const r of rows) { byId.set(r.id, r); seenDayId.add(r.id); tasdiqKun.set(r.id, r); }   // baza ustun
    days = [...byId.values()]; persist();
    // Faqat shu qurilmadagi (bazaga yetmagan) yozuvlarni yuqoriga sur
    keldi.add("kpi_day"); syncUp();
  },
  apply: ({ eventType, new: row, old }) => {
    if (eventType === "DELETE") { days = days.filter((d) => d.id !== old.id); tasdiqKun.delete(old.id); }
    else { const it = { ...row.data, id: row.id, staffId: row.staff_id, date: row.date };
      days = [...days.filter((d) => d.id !== it.id), it]; seenDayId.add(row.id); tasdiqKun.set(it.id, it); }
    persist();
  },
});
...
export function getPlan(staffId, month, typeOverride) {
  ...
  const mergeRules = (r) => ({ ...conf.defaultRules, ...tozaRules(r, type) });
...
// —— Kunlik yozuvlar ————————————————————————————
// Billz cheklari KPI jadvalidagi TO'RTTA pul ustunining manbasi:
// Savdo, Naqd, Payme va Servis. Ilgari to'rttasi ham qo'lda
// to'ldirilardi va aynan shu sabab ular ertami-kech bo'sh qolardi —
// 20-avgustdan 26-igacha hech kim yozmagan, cheklar esa har 30
// daqiqada kelib turgan.
//
// Bazadan o'lchandi (1–19 avgust, 37 kun-do'kon): menejer Billz
// raqamini QO'LDA KO'CHIRIB yozar ekan — servis 19 kundan 18 tasida
// tiyinigacha teng, naqd/payme ham. Ya'ni qo'l mehnati ma'lumot
// qo'shmasdi.
//
// IKKI XIL QOIDA — ATAYLAB:
//   • Savdo   — qo'lda yozilgani USTUN turadi (tarixiy KPI va bonuslar
//               jimgina qayta yozilmasin).
//   • Naqd/Payme/Servis — Billz ustun turadi (2026-08-26 qarori:
//               qo'lda kiritish butunlay olib tashlandi).
// Bazadagi eski qo'lda yozilgan qatorlar HECH QAYERDA O'CHIRILMAYDI —
// `kpi_day.data` da joyida turadi. Kamomad solishtiruvi
// (`kassaData.kassaControlDays`) ataylab o'shani, ya'ni XOM qo'lda
// yozilgan qatorni o'qiydi: aks holda ikkala tomon bir manba bo'lib,
// farq har doim 0 chiqardi. (Uning ekrani hozir menyuda yo'q — Excel
// yuklamasi bilan birga olib tashlangan, DAFTAR 14.10.)
//
// Davrda umuman chek bo'lmasa (demo rejim, API'dan oldingi tarix)
// qo'lda yozilgani ko'rsatiladi — `serviceIncome.servisKirim()` dagi
// "jonli bo'lsa jonli, bo'lmasa zaxira" naqshining aynan o'zi.
function billzKunlari(staffId, month) {
  const staff = getStaff(staffId);
  const type = getType(staffId);
  if (!staff?.storeId || !typeOf(type).parts.includes("sales")) return new Map();

  const out = new Map();
  // Hamma maydon `null` dan boshlanadi — "hali ma'lum emas" degani.
  // NOLDAN boshlanmaydi: kun `kunlikKirimStore` orqali ham paydo
  // bo'ladi (qarz to'lovi bo'lgan, chek bo'lmagan kun), va o'shanda
  // "Savdo" ustuni ishonch bilan `0` bo'lib qulflanardi. Og'ir
  // jadvallar fonda yuklanayotganda esa BUTUN oy shunday ko'rinardi —
  // bo'sh katak emas, ishonarli yolg'on.
  const kun = (date) => {
    let r = out.get(date);
    if (!r) { r = { sales: null, cash: null, payme: null, service: null }; out.set(date, r); }
    return r;
  };

  for (const sale of listSales()) {
    const date = ymd(sale.at);
    if (!date.startsWith(month) || sale.storeId !== staff.storeId) continue;
    const r = kun(date);
    r.sales = +((r.sales ?? 0) + (Number(sale.total) || 0)).toFixed(2);
  }

  // Naqd/Payme/Servis — kassaning kunlik kirimi bilan AYNAN bir
  // funksiyadan (CLAUDE.md 2026-08-14: bir tushuncha — bitta funksiya).
  // Aks holda KPI'da bir raqam, Kassada boshqasi turardi.
  const oyBoshi = `${month}-01`;
  const oyOxiri = `${month}-31`;
  for (const [date, w] of Object.entries(kunlikKirimStore(staff.storeId, oyBoshi, oyOxiri))) {
    const r = kun(date);
    r.cash = w.cash;
    r.payme = w.payme;
    r.service = w.service;
...
// Billz'dan keladigan ustunlar va ularning "avtomat" belgisi.
// Ekran shu ro'yxatdan o'qiydi — yangi ustun qo'shilsa u yerda
// alohida shart yozish kerak emas (CLAUDE.md 2026-08-06).
export const BILLZ_COLS = {
  sales: "billzSales",
  cash: "billzCash",
  payme: "billzPayme",
  service: "billzService",
};

const bosh = (v) => v === null || v === undefined || v === "";

export const listDays = (staffId, month) => {
  const manual = days.filter((d) => d.staffId === staffId && d.date.startsWith(month));
  const byDate = new Map(manual.map((d) => [d.date, d]));
  const billz = billzKunlari(staffId, month);

  for (const [date, auto] of billz) {
    const old = byDate.get(date);
    const row = { ...(old ?? { id: `billz:${staffId}:${date}`, staffId, date }) };

    // Savdo: qo'lda yozilgani ustun (tarix o'zgarmasin). `null` —
    // o'sha kunda chek yo'q, ya'ni qo'yadigan raqam ham yo'q.
    if (auto.sales !== null && bosh(old?.sales)) {
      row.sales = auto.sales;
      row[BILLZ_COLS.sales] = true;
    }

    // Naqd/Payme/Servis: Billz ustun. `null` bo'lsa o'sha kunda chek
    // ham, qarz to'lovi ham bo'lmagan — qo'lda yozilgani qoladi.
    for (const k of ["cash", "payme", "service"]) {
      if (auto[k] === null) continue;
      row[k] = auto[k];
      row[BILLZ_COLS[k]] = true;
    }
    byDate.set(date, row);
  }
...
export function saveDay(staffId, date, patch) {
  ...
  // Usta olgan pul KIRITILGAN KUNDAGI kurs bilan muhrlanadi — xarajat
  // (`rate_used`) kabi. Keyin bu summa kurs o'zgarganda suzmaydi
  // (`expensesData.installerPayouts`, DAFTAR 20 G).
  if ("olgan" in patch && Number(patch.olgan) > 0 && Number(patch.olgan) !== Number(cur?.olgan)) {
    next.olganKurs = getUsdRate() ?? cur?.olganKurs ?? null;
  }
