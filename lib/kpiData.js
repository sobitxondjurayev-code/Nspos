...
function billzKunlari(staffId, month) {
  ...
  const kun = (date) => {
    let r = out.get(date);
    if (!r) { r = { sales: 0, cash: null, payme: null, service: null }; out.set(date, r); }
    return r;
  };
  ...
  for (const sale of listSales()) {
    ...
    const r = kun(date);
    r.sales = +(r.sales + (Number(sale.total) || 0)).toFixed(2);
  }
...
export const listDays = (staffId, month) => {
  ...
  for (const [date, auto] of billz) {
    ...
    // Savdo: qo'lda yozilgani ustun (tarix o'zgarmasin)
    if (bosh(old?.sales)) { row.sales = auto.sales; row[BILLZ_COLS.sales] = true; }
