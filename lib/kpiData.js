...
// —— Kunlik yozuvlar ————————————————————————————
// Billz cheklari KPI jadvalidagi "Savdo"ning yagona avtomatik manbasi.
// Ilgari bu ustun qo'lda to'ldirilardi — 20-avgustdan keyingi 167 ta
// chek bazaga kelib turgan, KPI esa nol ko'rsatgan. Endi do'konga
// bog'langan menejer uchun kunlik savdo cheklarning o'zidan yig'iladi.
//
// Qo'lda yozilgan TARIX o'zgarmaydi: o'sha kuni `sales` bo'lsa u ustun.
// Qo'lda yozilmagan kunlarga esa Billz raqami chiqadi. Shu sabab
// o'tmishdagi KPI va bonuslar jimgina qayta yozilmaydi.
function billzSavdoKunlari(staffId, month) {
  ...
  for (const sale of listSales()) {
    ...
    out.set(date, +((out.get(date) ?? 0) + (Number(sale.total) || 0)).toFixed(2));
  }
  return out;
}
...
export const listDays = (staffId, month) => {
  ...
  const billz = billzSavdoKunlari(staffId, month);

  for (const [date, sales] of billz) {
    const old = byDate.get(date);
    // `null`/bo'sh katak — qo'lda savdo kiritilmagan, avtomat raqamni
    // qo'yamiz. Son bo'lsa tarixiy qo'lda kiritilgan qiymat saqlanadi.
    if (old?.sales !== null && old?.sales !== undefined && old?.sales !== "") continue;
    byDate.set(date, {
      ...(old ?? { id: `billz:${staffId}:${date}`, staffId, date }),
      sales,
      billzSales: true,
    });
  }
