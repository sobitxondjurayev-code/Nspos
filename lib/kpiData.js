...
registerModule("kpi_assign", {
  ...
  restore: (rows) => {
    ...
    for (const r of rows) { next[r.staff_id] = r.kpi_type; seenAssign.set(r.staff_id, r.kpi_type); }   // baza ustun
...
registerModule("kpi_plan", {
  ...
  restore: (rows) => {
    ...
    for (const r of rows) { byId.set(r.id, r); seenPlanId.add(r.id); }   // baza ustun
