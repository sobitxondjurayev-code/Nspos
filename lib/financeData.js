...
export function addOperation({ category, amount, method = "cash", note = "", customerId = null, storeId = null }) {
  ...
  // Do'kon BERILMASA — xodimning o'z do'koni (2026-09-03). Ilgari qarz
  // to'lovi, balans to'ldirish, ta'minotchi to'lovi `store_id = null`
  // bilan ketardi; RLS `can_see_store(NULL)` do'koni bor menejer/kassir
  // uchun NULL → rad. Rahbar (do'konsiz) uchun null qolaveradi.
  const doKon = storeId ?? getUser().storeId ?? null;
  const op = {
    id: "op" + seq,
    no: "OP-" + (100 + seq++),
    category,
