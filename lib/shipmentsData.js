...
export function addShipment(data) {
  ...
  // Baza HAQIQIY id qaytargach bola qatorlar o'sha id bilan (vaqtinchalik
  // id bilan ketsa uuid emas — rad bo'lardi, 2026-09-03)
  shipmentSync.created(item).then((r) => {
    if (r && !r.ok) return;
    const sp = getShipment(r?.data?.id ?? item.id);
    if (sp) saveShipmentChildren(sp);
  });
  return item;
}
...
export function applyShipment(id) {
  ...
  const storeId = shipment.storeId ?? asosiyDokonId();
