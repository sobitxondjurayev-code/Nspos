...
async function saveShipmentChildren(sp) {
  if (DEMO_MODE) return;
  await supabase.from("shipment_items").delete().eq("shipment_id", sp.id);
  await supabase.from("shipment_costs").delete().eq("shipment_id", sp.id);
  if (sp.items?.length) {
    await supabase.from("shipment_items").insert(sp.items.map((i) => ({
      shipment_id: sp.id, product_id: i.productId, qty: i.qty, unit_price: i.unitPrice,
    })));
  }
  if (sp.costs?.length) {
    await supabase.from("shipment_costs").insert(sp.costs.map((c) => ({
