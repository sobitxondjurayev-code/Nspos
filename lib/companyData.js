...
import { registerModule, update, rpc, DEMO_MODE, xotiraYangilandi } from "./db";
...
let company = { id: null, name: "", currency: "USD", usdRate: null, usdRateAuto: true, usdRateAt: null,
  ...
  reorderLeadDays: 14, reorderCoverDays: 30,
  // Biznes qoidalari (jsonb) — `sozlama()` orqali o'qiladi, kodda standart
  sozlamalar: {} };
...
const fromRow = (r) => ({
  ...
  // Buyurtma muddati (kun) — rahbar belgilaydi (`scripts/sql/reorder-days.sql`)
  reorderLeadDays: Number(r.reorder_lead_days) > 0 ? Number(r.reorder_lead_days) : 14,
  reorderCoverDays: Number(r.reorder_cover_days) > 0 ? Number(r.reorder_cover_days) : 30,
  sozlamalar: r.sozlamalar && typeof r.sozlamalar === "object" ? r.sozlamalar : {},
});

registerModule("companies", {
  // `ledger_start` ALBATTA ro'yxatda bo'lsin: u yo'q bo'lsa fromRow
  // uni joriy oy boshi deb oladi va 1-sentabrda butun avgust hisobi
  // birdaniga yo'qoladi (kassa balansi, P&L, balans — hammasi
  // "shu oydan" bo'lib qoladi). 2026-08-14 gacha shu ustun so'ralmagan
  // edi, xato faqat oy boshida ko'rinar edi. `sozlamalar` ham shunday.
  table: "companies", select: "id,name,currency,usd_rate,usd_rate_auto,usd_rate_at,service_names,ledger_start,reorder_lead_days,reorder_cover_days,sozlamalar", realtime: true,
  fromRow,
  restore: (rows) => {
    if (!rows[0]) return;
...
// Baza rad etsa (faqat rahbar yoza oladi) xotira va localStorage
// ORQAGA qaytadi. Ilgari `silent` edi va lokal nusxa yangi qiymatda
// qolardi: menejer sozlamani "o'zgartirdi", boshqa qurilmada esa eski
// turardi (2026-09-03).
function save(patch) {
  const oldingi = company;
  company = { ...company, ...patch };
  persist();
  if (DEMO_MODE || !company.id) return Promise.resolve({ ok: true, demo: true });
  return update("companies", company.id, {
    ...
    reorder_lead_days: company.reorderLeadDays,
    reorder_cover_days: company.reorderCoverDays,
    sozlamalar: company.sozlamalar ?? {},
  }, "Kompaniya sozlamasi").then((r) => {
    if (r && !r.ok) { company = oldingi; persist(); xotiraYangilandi(); }
    return r;
  });
}
...
// ══════════════════════════════════════════════════════════════
// Kodda qotgan raqamlar (usta pul kunlari, kam qoldiq chegarasi, qarz
// guruhlari, AR yosh guruhlari, sotuv oynasi) `companies.sozlamalar`
// (jsonb) dan o'qiladi; kalit yo'q bo'lsa kodda turgan STANDART.
// O'qish CHAQIRUV vaqtida (`sozlama("kpi.payDays", [...])`) — modul
// darajasida muzlatilmaydi. Ekran, `tekshir`, API va bot bir xil
// qiymatni oladi (hammasi `companies` ni yuklaydi).
export function sozlama(yol, standart) {
  const v = String(yol).split(".").reduce(
    (o, k) => (o && typeof o === "object" && k in o ? o[k] : undefined), company.sozlamalar ?? {});
  return v === undefined || v === null ? standart : v;
}
export function setSozlama(yol, qiymat) {
  const next = JSON.parse(JSON.stringify(company.sozlamalar ?? {}));
  ...
  if (qiymat === undefined || qiymat === null) delete o[parts.at(-1)];
  else o[parts.at(-1)] = qiymat;
  return save({ sozlamalar: next });
}
// Musbat butun sonlar ro'yxati ("1, 5, 10" → [1,5,10]); bo'sh bo'lsa standart
export const sonlarRoyxati = (matn, standart) => {
  const xs = String(matn ?? "").split(/[,\s;]+/).map((x) => Math.round(Number(x))).filter((n) => Number.isFinite(n) && n > 0);
  return xs.length ? [...new Set(xs)].sort((a, b) => a - b) : standart;
};
...
// Tovar nomi xizmatmi
// —— Buyurtma muddati ————————————————————————————————
// `analytics.reorderList()` standart qiymatni shu yerdan oladi — ekran,
// REST API va Telegram bot bir xil muddat bilan hisoblaydi. Rahbar
// "Qoldiq salomatligi" sahifasida o'zgartiradi (2026-09-02 qarori:
// avtomat 14+30 emas, o'zi belgilaydi).
export const getReorderDays = () => ({
  lead: Number(company.reorderLeadDays) > 0 ? Number(company.reorderLeadDays) : 14,
  cover: Number(company.reorderCoverDays) > 0 ? Number(company.reorderCoverDays) : 30,
});
const kunChegara = (v, dflt, eng = 1) => {
  const n = Math.round(Number(v));
  return Number.isFinite(n) && n >= eng && n <= 365 ? n : dflt;
};
// —— Sotuv tezligi oynasi ————————————————————————————
// "Kunlik o'rtacha necha kunlik tarixdan olinadi". BITTA ta'rif:
// Sozlamalar → Biznes qoidalari ham, "Qoldiq salomatligi" sahifasidagi
// maydon ham shu yerdan o'qiydi va shu yerga yozadi — ikki joyda ikki
// xil chegara bo'lib qolmasin.
//
// Eng kami 7 kun: undan qisqa oynada bitta katta chek "kuniga 5 dona
// sotilyapti" degan yolg'on tezlik beradi va buyurtma miqdori shunga
// qarab shishadi. Zaxira oyna — shuning UCH BAROBARI
// (`analytics.stockCoverage`), ya'ni u ham shu raqamdan kelib chiqadi.
export const getStockWindow = () => kunChegara(sozlama("stock.windowDays", 30), 30, 7);

export function setStockWindow(kun) {
  return setSozlama("stock.windowDays", kunChegara(kun, getStockWindow(), 7));
}

export function setReorderDays({ lead, cover } = {}) {
  const hozir = getReorderDays();
  // Natija (va'da) qaytadi — Sozlamalar "Saqlandi/Saqlanmadi" ni ko'rsatadi
  return save({
    reorderLeadDays: kunChegara(lead ?? hozir.lead, hozir.lead),
    reorderCoverDays: kunChegara(cover ?? hozir.cover, hozir.cover),
  });
}
