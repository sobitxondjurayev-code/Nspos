...
registerModule("companies", {
  ...
  // edi, xato faqat oy boshida ko'rinar edi.
  table: "companies", select: "id,name,currency,usd_rate,usd_rate_auto,usd_rate_at,service_names,ledger_start,reorder_lead_days,reorder_cover_days", realtime: true,
  fromRow,
  restore: (rows) => {
    if (!rows[0]) return;
