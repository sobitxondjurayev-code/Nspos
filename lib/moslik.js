...
import { totalExpenses, expensesByCategory, expensesByStore, expensesInRange, kelishXarajati, tannarxgaMi } from "./expensesData";
import { payrollCost } from "./payrollData";
import { profitAndLoss, cashFlow, salesPnl, foydaPulKoprigi } from "./pnlData";
...
import { listDebts, paidOf, overallDebtStats, jamiQarz, debtorRowsDavr, jadvalHozirgiQarz } from "./debtsData";
import { arAging, stockCoverage, productProfit } from "./analytics";
import { listStores, skladId } from "./storesData";
...
import { computeMonth, monthKey } from "./kpiData";
import { boardCameras, monthBounds, boardRows } from "./installerBoard";
...
function netProfitEverywhere() {
  ...
  const qoldiq = +(k.grossProfit - k.opex - k.wages - (k.soliq ?? 0)).toFixed(2);
  if (!same(qoldiq, k.netProfit)) {
    out.push({
      id: "dash-row", level: "error",
      title: `Rahbariyat paneli: ${money(k.grossProfit)} − ${money(k.opex + k.wages)} − soliq ${money(k.soliq ?? 0)} = ${money(qoldiq)}, kartochkada esa ${money(k.netProfit)}`,
...
//
// 2026-09-04 (DAFTAR 20): `salesData.fromRow` ishorani O'QISHDA tuzatib
// qo'ygani uchun bu tekshiruv `s.total` ga qarab HECH QACHON qizarmasdi —
// u tuzatilgan qiymatni tekshirardi. Endi XOM Billz summasi (`totalXom`)
// bilan: ishora tuzatilishi faqat ALMASHUV chekida kutiladi (Billz
// almashuvda farqni ishorasiz beradi); sotuv/qaytarish chekida ishora
// teskari bo'lsa — haqiqiy nosozlik. Almashuv tuzatishlari sanaladi,
// lekin xato emas.
function exchangeSignCheck() {
  ...
  for (const s of listSales()) {
    const xom = s.totalXom == null ? +s.total || 0 : +s.totalXom || 0;
    ...
    if (xom === 0 || sub === 0) continue;
    if (Math.sign(xom) === Math.sign(sub)) continue;
    if (s.type === "exchange") continue;          // kutilgan — o'qishda tuzatiladi
    bad++;
    const gap = Math.abs(xom - sub);
    tasir += xom - sub;
    if (!worst || gap > worst.gap) worst = { no: s.no, at: s.at, total: xom, sub, gap };
  }

  if (bad) {
    out.push({
      id: "sale-sign", level: "error",
      title: `${bad} ta sotuv/qaytarish chekida Billz summasi ishorasi qatorlarga teskari (${money(tasir)})`,
      detail: `Eng kattasi: ${worst.no} — chek ${money(worst.total)}, qatorlar ${money(worst.sub)}. `
        + "Ishora tuzatish faqat almashuv chekida kutiladi; bu yerda chek turi boshqa — "
        + "`billzMap.saleType` yoki Billz ma'lumoti tekshirilsin.",
      action: "Sotuvlarni ochish", href: "/sales",
    });
  }
  return out;
...
// —— Xizmat qatori tannarxsiz ——————————————————————————
// DAFTAR 20 (A): Billz "montaj" tovariga 5 $ tannarx qo'ygan — u ustaning
// haqiqiy puli emas. Usta puli ish haqida (`kpiPayroll`) chegiriladi;
// tannarxga ham kirsa xizmat xarajati IKKI MARTA sanaladi (avgust 2026:
// 5 065 $). `salesData.qatorTannarx` xizmat qatoriga 0 beradi — bu
// tekshiruv ilovaning o'z "Tovar kesimida foyda" hisobi orqali shuni
// tasdiqlaydi: xizmat tovarining tannarxi 0 va marjasi 100 %.
function servisTannarxNol() {
  const out = [];
  const boshiSana = boshi(), bugunSana = bugun();
  const xizmat = productProfit(boshiSana, bugunSana, { by: "product" })
    .filter((r) => r.product?.isService);
  const yomon = xizmat.filter((r) => Math.abs(r.cogs) > 0.005);
  if (yomon.length) {
    out.push({
      id: "servis-tannarx", level: "error",
      title: `${yomon.length} ta xizmat tovarining tannarxi 0 emas — xizmat xarajati ikki marta sanalmoqda`,
      detail: yomon.slice(0, 3).map((r) => `${r.name}: tannarx ${money(r.cogs)}`).join(" · ")
        + ". Usta puli ish haqida chegiriladi; Billz'dagi tannarx o'qilmasligi kerak (`salesData.qatorTannarx`).",
      action: "Tovar kesimida foydani ochish", href: "/reports/product_profit",
    });
  }
  return out;
}
...
// —— Qarz yoshi: guruhlar = jami, muddat bo'linishi = jami, AR = jami ————
// DAFTAR 20 B: yosh guruhlari `debtsData.arBuckets` — Qarzdorlik
// sahifasi, AR aging, API, bot bitta ro'yxat. Guruhlar yig'indisi,
// "muddati o'tgan + kelmagan" va AR aging `open` — uchalasi `jamiQarz`
// jamisiga teng bo'lishi shart.
function qarzYoshGuruhlari() {
  const out = [];
  const j = jamiQarz();
  const yosh = +j.yosh.reduce((s, g) => s + g.summa, 0).toFixed(2);
  const muddat = +(j.muddatiOtgan.summa + j.muddatiKelmagan.summa).toFixed(2);
  const chegara = 0.005 * (j.yosh.length + 1) + 0.001;
  if (Math.abs(yosh - j.jami) > chegara) {
    out.push({
      id: "qarz-yosh", level: "error",
      title: `Qarz yosh guruhlari yig'indisi jamiga teng emas`,
      detail: `Guruhlar ${money(yosh)}, jami ${money(j.jami)}. Bir qarz guruhga tushmay qolgan.`,
      action: "Qarzdorlikni ochish", href: "/finance/debts",
    });
  }
  if (Math.abs(muddat - j.jami) > 0.011) {
    out.push({
      id: "qarz-muddat", level: "error",
      title: `"Muddati o'tgan + kelmagan" jamiga teng emas`,
      detail: `${money(j.muddatiOtgan.summa)} + ${money(j.muddatiKelmagan.summa)} ≠ ${money(j.jami)}.`,
      action: "Qarzdorlikni ochish", href: "/finance/debts",
    });
  }
  const a = arAging();
  if (!same(a.open, j.jami)) {
    out.push({
      id: "qarz-ar", level: "error",
      title: `AR aging ochiq qarzi "Jami qarzdorlik"ga teng emas`,
      detail: `AR aging ${money(a.open)}, jami ${money(j.jami)}.`,
      action: "Qarz yoshini ochish", href: "/reports/ar_aging",
    });
  }
  const shubhali = a.buckets.at(-1)?.open ?? 0;
  if (!same(shubhali, j.shubhali.summa)) {
    out.push({
      id: "qarz-shubhali", level: "error",
      title: `Shubhali qarz: Qarzdorlik ${money(j.shubhali.summa)}, AR aging ${money(shubhali)}`,
      detail: "Ikkalasi oxirgi yosh guruhi — bitta ro'yxatdan chiqishi kerak.",
      action: "Qarzdorlikni ochish", href: "/finance/debts",
    });
  }
  return out;
...
// —— Foyda → Pul ko'prigi: qatorlar = kutilgan; sof foyda = P&L ————
// —— Tovar kelish xarajati: OPEX dan tannarxga o'tdi, yo'qolmadi ——
// Menejerlar so'rovi (2026-09-05): yo'lkira/dostavka tannarxga qo'shilsin.
// Pul bir qatordan ikkinchisiga KO'CHADI — ya'ni sof foyda o'zgarmasligi
// SHART. Uch savol:
//   1. P&L yo'lkirani o'zicha hisoblamayaptimi (bitta ta'rif);
//   2. OPEX dan chiqqan pul yo'qolib ketmadimi;
//   3. sof foyda eski usul bilan hisoblanganda ham o'shami — yo'lkira
//      IKKI MARTA ayirilmayaptimi (eng ehtimolli regressiya: kimdir
//      `expensesPnl` `total` iga `+ kelish` qo'shib qo'yishi).
function kelishTannarxda() {
  const out = [];
  const a = boshi(), b = bugun();
  const p = profitAndLoss(a, b);
  const k = kelishXarajati(a, b);

  if (!same(p.cogs.kelish, k.total)) {
    out.push({
      id: "kelish-tarif", level: "error",
      title: `Kelish xarajati: P&L ${money(p.cogs.kelish)}, Xarajatlar ${money(k.total)}`,
      detail: "Ikkalasi `expensesData.kelishXarajati()` dan kelishi kerak — P&L o'z filtrini yozib qo'ygan.",
      action: "P&L ni ochish", href: "/finance/pnl",
    });
  }

  // OPEX + kelish = oyliksiz jami xarajat (hech narsa yo'qolmagan)
  const oyliksiz = +expensesInRange(a, b)
    .filter((e) => e.category !== "salary")
    .reduce((x, e) => x + e.amount, 0).toFixed(2);
  const yig = +(p.expenses.opex + p.cogs.kelish).toFixed(2);
  if (!same(yig, oyliksiz)) {
    out.push({
      id: "kelish-opex", level: "error",
      title: `OPEX ${money(p.expenses.opex)} + kelish ${money(p.cogs.kelish)} = ${money(yig)}, xarajatlar esa ${money(oyliksiz)}`,
      detail: "Yo'lkira OPEX dan chiqdi-yu, tannarxga kirmadi (yoki teskarisi) — pul yo'qolgan.",
      action: "Xarajatlarni ochish", href: "/finance/expenses",
    });
  }

  // Sof foyda invariantligi: yo'lkirani xarajat deb hisoblasak ham o'sha
  const eski = +(p.revenue.total
    - (p.cogs.goods + p.cogs.serviceMaterials)
    - (p.expenses.total + p.cogs.kelish + p.expenses.installerShare)
    - (p.soliq?.summa ?? 0)).toFixed(2);
  if (!same(eski, p.netProfit)) {
    out.push({
      id: "kelish-foyda", level: "error",
      title: `Sof foyda ${money(p.netProfit)}, yo'lkirasiz usul ${money(eski)} — farq ${money(+(p.netProfit - eski).toFixed(2))}`,
      detail: "Kelish xarajati ikki marta ayirilgan: u tannarxda ham, `expenses.total` ichida ham turibdi.",
      action: "P&L ni ochish", href: "/finance/pnl",
    });
  }

  // Ekranda xarajat qatori bo'lib ham ko'rinmasin
  const qator = Object.keys(p.expenses.byCategory ?? {});
  const sizib = (k.byCategory ?? []).filter((c) => qator.includes(c.label));
  if (sizib.length) {
    out.push({
      id: "kelish-qator", level: "error",
      title: `Kelish turi xarajat qatorida ham ko'rinmoqda: ${sizib.map((c) => c.label).join(", ")}`,
      detail: "`expensesPnl` dagi `opex` filtri `tannarxgaMi` ni o'tkazib yuborgan.",
      action: "P&L ni ochish", href: "/finance/pnl",
    });
  }
  return out;
...
// DAFTAR 20 (R): ko'prik "izohlanmagan" qatorini YASHIRMAYDI, lekin
// uning o'z arifmetikasi buzilmasin — qatorlar yig'indisi "kutilgan"ga,
// birinchi qator P&L sof foydasiga teng bo'lishi shart.
function koprikArifmetikasi() {
  const out = [];
  const a = boshi(), b = bugun();
  const k = foydaPulKoprigi(a, b);
  const yig = +k.qatorlar.reduce((s, r) => s + r.summa, 0).toFixed(2);
  if (!same(yig, k.kutilgan)) {
    out.push({
      id: "koprik-yigindi", level: "error",
      title: `Foyda → Pul: qatorlar ${money(yig)}, "kutilgan" ${money(k.kutilgan)}`,
      detail: "Ko'prik qatorlari yig'indisi kutilgan kassa o'zgarishiga teng bo'lishi kerak.",
      action: "P&L → Foyda → Pul", href: "/finance/pnl",
    });
  }
  const p = profitAndLoss(a, b).netProfit;
  if (!same(k.qatorlar[0]?.summa, p)) {
    out.push({
      id: "koprik-foyda", level: "error",
      title: `Foyda → Pul birinchi qatori ${money(k.qatorlar[0]?.summa)}, P&L sof foydasi ${money(p)}`,
      detail: "Ko'prik P&L dan boshlanadi — bitta raqam.",
      action: "P&L → Foyda → Pul", href: "/finance/pnl",
    });
  }
  return out;
...
// —— Balans: aktiv − majburiyat − kapital − izohlanmagan = 0; kapital = P&L ——
// DAFTAR 20 (K): kapital tiqin emas — boshlang'ich + yig'ilgan foyda − NS.
// Tenglama "izohlanmagan" bilan yopiladi (arifmetika), yig'ilgan foyda
// esa P&L (hisob boshidan bugungacha) bilan AYNAN teng.
function balansKapital() {
  const out = [];
  const b = balanceSheet(new Date());
  const k = b.kapitalTafsilot;
  const tenglama = +(b.totalAssets - b.totalLiabilities - b.equity - (b.izohlanmagan ?? 0)).toFixed(2);
  if (Math.abs(tenglama) > 0.011) {
    out.push({
      id: "balans-tenglama", level: "error",
      title: `Balans tenglamasi yopilmaydi: ${money(tenglama)}`,
      detail: `Aktiv ${money(b.totalAssets)} − majburiyat ${money(b.totalLiabilities)} − kapital ${money(b.equity)} − izohlanmagan ${money(b.izohlanmagan)}.`,
      action: "Balansni ochish", href: "/finance/balance",
    });
  }
  const p = profitAndLoss(boshi(), bugun()).netProfit;
  if (k && !same(k.yigilganFoyda, p)) {
    out.push({
      id: "balans-kapital", level: "error",
      title: `Balans "yig'ilgan foyda" ${money(k.yigilganFoyda)}, P&L (hisob boshidan) ${money(p)}`,
      detail: "Kapitaldagi foyda P&L dan o'qilishi kerak — bitta raqam.",
      action: "Balansni ochish", href: "/finance/balance",
    });
  }
  return out;
...
// —— Qarzdorlik jadvali "Hozirgi qarzi" jamisi = "Jami qarzdorlik" ——
// Jadval davr bo'yicha oqim ustunlari bilan chiziladi (2026-09-03),
// lekin "Hozirgi qarzi" ustuni snapshot — uning jamisi kartochka
// (`jamiQarz`) bilan TENG bo'lishi shart, qaysi davr tanlanmasin.
// Qator har mijoz uchun alohida yaxlitlanmaydi — xom yig'indidan
// (DAFTAR 17.4: alohida yaxlitlash 125 mijozda 3 tiyin farq bergan).
function qarzJadvalVsJami() {
  const out = [];
  const rows = debtorRowsDavr(getLedgerStart(), new Date());
  const jadval = jadvalHozirgiQarz(rows);
  const j = jamiQarz().jami;
  if (!same(jadval, j)) {
    out.push({
      id: "qarz-jadval", level: "error",
      title: `Qarzdorlik jadvali "Hozirgi qarzi" jamisi kartochkaga teng emas`,
      detail: `Jadval ${money(jadval)}, "Jami qarzdorlik" ${money(j)}, farq ${money(jadval - j)}. Bir qarz qatorga tushmay qolgan.`,
      action: "Qarzdorlikni ochish", href: "/finance/debts",
    });
  }
  return out;
...
// ══════════════════════════════════════════════════════════════
// RO'YXAT
// —— 24. Usta puli: bitta hamyondan, bitta raqam ————————
// Rahbar fidbegi (2026-09-03): usta puli DO'KON servis kassasidan
// yozilardi, u esa har kuni nol qilib topshiriladi (`kassa_ops`
// transfer → kompaniya balansi). Natijada hamyon minusga tushib,
// menejer yangi xarajat umuman kirita olmay qolardi.
//
// Ikki narsa tekshiriladi:
//   1. Usta puli "Oylik maoshlar" ustuniga TO'LIQ tushganmi
//      (DAFTAR 68 formulasi: servis kirimi − (svcExp + salary)
//      = kassadagi servis puli).
//   2. Hammasi BITTA hamyondan chiqyaptimi. Bir nechta bo'lsa —
//      demak yana do'kon hamyoniga tarqalyapti va o'sha do'kon
//      ertami-kechmi minusga tushadi.
function ustaPuliBirHamyondan() {
  const a = boshi(), b = bugun();
  const usta = expensesInRange(a, b).filter((e) => e.source === "installer");
  const out = [];
  if (!usta.length) return out;

  // "Oylik maoshlar" ustuni SERVIS hamyonidan chiqqan HAR OYLIKNI
  // oladi (`kassaData` → `category === "salary" && method === "service"`),
  // ya'ni usta puli + menejer qo'lda kiritgan servis oyligi. Faqat usta
  // puli bilan solishtirish XATO edi: 2026-09-04 da qo'lda kiritilgan
  // 14 ta yozuv (512.66 $) tufayli tekshiruv bekorga qizarardi.
  const oylik = expensesInRange(a, b)
    .filter((e) => e.category === "salary" && e.method === "service");
  const jami = oylik.reduce((s, e) => s + e.amount, 0);
  const ustun = summaryRow(a, b).salary ?? 0;
  if (!same(jami, ustun)) {
    out.push({
      id: "usta-oylik", level: "error",
      title: `Servis oyligi: yozuvlarda ${money(jami)}, "Oylik maoshlar" ustunida ${money(ustun)}`,
      detail: "Ikkalasi ham bitta shartdan chiqadi (`category = salary`, `method = service`) — farq bo'lsa ustun shartida xato bor.",
      action: "Kassani ochish", href: "/finance/kassa",
    });
  }

  const hamyonlar = [...new Set(usta.map((e) => e.kassa))];
  if (hamyonlar.length > 1) {
    out.push({
      id: "usta-hamyon", level: "error",
      title: `Usta puli ${hamyonlar.length} xil kassadan chiqyapti`,
      detail: `Kassalar: ${hamyonlar.join(", ")}. Servis kassasi har kuni nol qilib topshiriladi — pul topshirilgan joydan (kompaniya balansi) chiqishi kerak, aks holda hamyon minusga tushadi.`,
      action: "Kassani ochish", href: "/finance/kassa",
    });
  }
  return out;
...
// —— 25. Ustalar reytingi: ko'rinish = KPI kunligi ————————
// Kamera soni endi IKKI joydan chiqadi:
//   • `installer_cameras` ko'rinishi — reyting jadvali shundan
//     o'qiydi (usta boshqa ustani ko'rishi uchun; RLS `kpi_day` ni
//     unga bermaydi)
//   • `kpiData.computeMonth` — bonus va oylik hisobi shundan
//
// Ikkalasi bitta `kpi_day.data.cameras` dan chiqadi, ya'ni HAR DOIM
// teng bo'lishi shart. Farq chiqsa — ko'rinish eskirgan yoki filtri
// noto'g'ri, va reytingda bir raqam, oylikda boshqasi turadi.
function ustaKameraKorinish() {
  const out = [];
  // Ko'rinish umuman yuklanmagan bo'lsa (jadval yo'q, huquq yo'q) —
  // "hammasi 0" degan soxta xato bermaymiz, buni alohida aytamiz.
  if (boardRows() === 0) return out;

  const oy = monthKey(new Date());
  const { from, to } = monthBounds(oy);
  for (const s of listStaff().filter((x) => x.role === "installer" && x.active !== false)) {
    const korinish = boardCameras(s.id, from, to);
    const kpi = computeMonth(s.id, oy, "installer").cameras;
    if (korinish === kpi) continue;
    out.push({
      id: `usta-kamera-${s.id}`, level: "error",
      title: `${s.name}: reytingda ${korinish} kamera, KPI kunligida ${kpi}`,
      detail: "Ikkalasi ham `kpi_day.data.cameras` dan chiqadi. Farq bo'lsa `installer_cameras` ko'rinishi yoki uning filtri noto'g'ri — reytingdagi va oylikdagi raqam bir-biriga qarshi chiqadi.",
      action: "Ustalar reytingini ochish", href: "/installers",
    });
  }
  return out;
}

// —— 26. Ombor qoplamasi: do'konlar yig'indisi = har do'kon alohida —
// 2026-09-04 dan do'kon BIR NECHTA tanlanadi (`stockCoverage({ storeIds })`).
// Yig'indi har do'konning alohida raqamidan chiqishi shart — aks holda
// "Optim + Namangan" tanlagan rahbar Tovarlar sahifasidagi qoldiqqa
// qarshi raqam ko'radi. Hamma do'kon tanlansa "Barchasi" bilan teng.
// "Skladda" ustuni: tanlovda Sklad BO'LMAGANDA — Sklad qoldig'i,
// bo'lganda — yo'q (u allaqachon yig'indida).
function omborDokonYigindisi() {
  const out = [];
  const dokonlar = listStores().map((x) => x.id);
  if (dokonlar.length < 2) return out;
  const now = new Date();                 // hamma chaqiruv BIR vaqtga
  const qoldiq = (r) => new Map(r.map((x) => [x.product.id, x]));
  const alohida = new Map(dokonlar.map((id) => [id, qoldiq(stockCoverage({ storeId: id, now }))]));
  const hammasi = qoldiq(stockCoverage({ now }));
  const sklad = skladId();
  const sinov = [
    { nom: "hamma do'kon", ids: dokonlar, hammasiBilan: true },
    // Sklad bo'lmagan ikkita do'kon — "Skladda" ustuni chiqadigan hol
    { nom: "ikki filial", ids: dokonlar.filter((id) => id !== sklad).slice(0, 2), hammasiBilan: false },
  ].filter((t) => t.ids.length >= 2);

  for (const t of sinov) {
    const birga = stockCoverage({ storeIds: t.ids, now });
    let farq = 0, skladFarq = 0, misol = null;
    for (const r of birga) {
      const kutilgan = t.ids.reduce((a, id) => a + (alohida.get(id).get(r.product.id)?.stock ?? 0), 0);
      const umumiy = hammasi.get(r.product.id)?.stock ?? 0;
      if (Math.abs(r.stock - kutilgan) > 1e-6 || (t.hammasiBilan && Math.abs(r.stock - umumiy) > 1e-6)) {
        farq++;
        misol ??= `${r.product.name}: birga ${r.stock}, alohida yig'indisi ${kutilgan}${t.hammasiBilan ? `, "Barchasi" ${umumiy}` : ""}`;
      }
      const skladKutilgan = sklad && !t.ids.includes(sklad) ? (alohida.get(sklad)?.get(r.product.id)?.stock ?? 0) : null;
      const bor = r.skladda != null, kutBor = skladKutilgan != null;
      if (bor !== kutBor || (bor && Math.abs(r.skladda - skladKutilgan) > 1e-6)) skladFarq++;
    }
    if (farq) out.push({
      id: `ombor-dokon-${t.ids.join("+")}`, level: "error",
      title: `Ombor qoplamasi (${t.nom}): ${farq} tovarda yig'indi alohida raqamlarga teng emas — ${misol}`,
      detail: "`stockCoverage({ storeIds })` qoldiqni tanlangan do'konlar bo'yicha yig'adi. Farq bo'lsa yig'ish yoki sotuv filtri noto'g'ri — rahbar Tovarlar sahifasidagi qoldiqqa qarshi raqam ko'radi.",
      action: "Ombor qoplamasini ochish", href: "/reports/stock_coverage",
    });
    if (skladFarq) out.push({
      id: `ombor-sklad-${t.ids.join("+")}`, level: "error",
      title: `Ombor qoplamasi (${t.nom}): ${skladFarq} tovarda "Skladda" ustuni noto'g'ri`,
      detail: "Tanlovda Sklad bo'lmasa \"Skladda\" = Sklad qoldig'i, bo'lsa — yo'q (yig'indida). Aks holda rahbar buyurtma o'rniga ko'chirishni (yoki aksincha) ko'radi.",
      action: "Ombor qoplamasini ochish", href: "/reports/stock_coverage",
    });
  }
  return out;
...
export const CHECKS = [
  ...
  { id: "sale-sign", name: "Chek summasi ishorasi qatorlarga mos (almashuvdan tashqari)", run: exchangeSignCheck },
  { id: "servis-tannarx", name: "Xizmat (montaj) qatori tannarxsiz — usta puli ish haqida", run: servisTannarxNol },
  ...
  { id: "qarz-yosh", name: "Qarz yoshi: guruhlar = muddat bo'linishi = AR aging = jami", run: qarzYoshGuruhlari },
  { id: "koprik", name: "Foyda → Pul ko'prigi: qatorlar = kutilgan, bosh qator = P&L", run: koprikArifmetikasi },
  { id: "kelish-tannarx", name: "Kelish xarajati: OPEX dan tannarxga o'tdi, sof foyda o'zgarmadi", run: kelishTannarxda },
  { id: "balans-kapital", name: "Balans: kapital = boshlang'ich + P&L − NS; tenglama izohlanmagan bilan yopiladi", run: balansKapital },
  { id: "qarz-jadval", name: "Qarzdorlik jadvali jamisi = Jami qarzdorlik", run: qarzJadvalVsJami },
  ...
  { id: "usta-puli", name: "Usta puli: bitta hamyondan, \"Oylik maoshlar\"ga teng", run: ustaPuliBirHamyondan },
  { id: "usta-kamera", name: "Ustalar reytingi: ko'rinish = KPI kunligi", run: ustaKameraKorinish },
  { id: "ombor-dokon", name: "Ombor qoplamasi: do'konlar yig'indisi = har do'kon alohida", run: omborDokonYigindisi },
];
