...
//
// Token modul yuklanganda BIR MARTA o'qiladi. Kirish va chiqishdan
// keyin sahifa TO'LIQ qayta yuklanadi (`location.href`), ya'ni
// modul ham yangi token bilan qaytadan boshlanadi. Shu tufayli
// 50 ta modulga tegmasdan ishlaydi.
const token = typeof document !== "undefined" ? tokenOl() : null;
...
export const supabase = DEMO_MODE ? null : createClient(url, kalit, {
  auth: {
  ...
  global: { headers: { Authorization: `Bearer ${kalit}` } },
});
  ...
  async function tekshir() {
    ...
    for (const m of realtimeModullar) {
      try {
        ...
        // Tarmoq uzilsa jim o'tamiz: keyingi urinishda tuzaladi
      }
    }
  }
