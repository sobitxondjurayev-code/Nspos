...
// ══════════════════════════════════════════════════════════════
// CHEK QATORINING TANNARXI — bitta qoida, hamma hisob shundan
// ══════════════════════════════════════════════════════════════
// P&L (`pnlData.salesPnl`), rahbariyat paneli (`managementData.
// billzInRange`) va "Tovar kesimida foyda" (`analytics.productProfit`)
// ilgari har biri o'zicha `i.costPrice || katalog || 0` yozardi.
// 2026-09-04 auditi (DAFTAR 20) ikki xatoni ko'rsatdi:
//
//   1. XIZMAT (montaj) qatorida Billz tannarxi 5 $ turadi — bu ustaning
//      haqiqiy puli emas, taxminiy raqam. Usta puli ish haqida
//      (`payrollData.kpiPayroll`) allaqachon chegiriladi; 5 $ ham
//      tannarxga kirsa xizmat xarajati IKKI MARTA sanaladi — avgustda
//      1 013 montaj × 5 = 5 065 $ sof foyda kam ko'ringan.
//   2. Tannarx 0/null bo'lsa `|| 0` uni "bepul" qilib qo'yadi va o'sha
//      qator 100 % foyda beradi. `analytics.stockCoverage` buni
//      allaqachon rad etgan edi ("0 bepul degani") — bu yerda ham
//      NOMA'LUM = null, va u alohida sanaladi, pulga kirmaydi.
//
// Qaytaradi: { tannarx: number|null, manba: "xizmat"|"qator"|"katalog"|null }
//   xizmat  — `products.is_service` yoki `companies.service_names` bo'yicha
//   qator   — `sale_items.cost_price` (sinxron paytidagi katalog tannarxi;
//             Billz chek qatorida tannarx bermaydi — `billzMap.saleItemRows`)
//   katalog — hozirgi `products.cost_price`
export function qatorTannarx(item, product) {
  if (product?.isService || isServiceName(item?.name)) return { tannarx: 0, manba: "xizmat" };
  if (+item?.costPrice > 0) return { tannarx: +item.costPrice, manba: "qator" };
  if (+product?.costPrice > 0) return { tannarx: +product.costPrice, manba: "katalog" };
  return { tannarx: null, manba: null };
}
...
const saleSync = syncTable("sales", {
  ...
  // ── ILOVA OCHILGANDA HAMMA CHEK BIRDAN KELMAYDI (2026-09-12) ──
  // Bu ilovadagi eng og'ir yuk: 9 600 chek va ular ichidagi 35 000
  // qator — qolgan hamma jadval yig'indisidan katta. Shuning uchun
  // avval OXIRGI 120 KUN keladi (bosh sahifa, KPI va kassa davrlari
  // shu ichida), qolgan tarix esa orqadan.
  //
  // Xavfi bitta va u aniq: ikkinchi to'lqingacha "Yil" yoki butun
  // tarix bo'yicha raqam KAM chiqadi. Shuning uchun oraliqdagi
  // holatni faqat DAVR bo'yicha hisoblaydigan sahifa ishlatadi
  // (`useDavrToliq(range.from)`); qolgan hamma sahifa avvalgidek
  // to'liq yuklanishni kutadi (`useToliq`).
  oyna: { ustun: "sold_at", kun: 120 },
  get: () => sales,
  // ── ALMASHTIRILGAN CHEK XOTIRAGA KIRMAYDI ──
  // Excel importidan qolgan 39 chek Billz'dagi AYNAN o'sha chekning
  // nusxasi edi (raqam bir xil, summa bir tiyinga farq qilardi —
  // shuning uchun `linkSales()` ularni bog'lay olmagan). Ikkalasi
  // birga sanalsa tarixiy tushum 17 052.88 $ ga oshib ketardi va
  // "39 ta chekda tovar tarkibi yo'q" ogohlantirishi shundan chiqardi.
  //
  // Bazada ular O'CHIRILMAYDI — `superseded_by` havolasi bilan
  // joyida turadi (`scripts/sql/sales-superseded.sql`). Bu yerda esa
  // xotiraga umuman kiritilmaydi: `salesInRange`, P&L, ABC va
  // qolgan hamma hisob to'g'ridan-to'g'ri `sales` massividan o'qiydi,
  // ya'ni bitta joyda to'sish yetarli.
  set: (v) => { sales = v.filter((s) => !s.supersededBy); },
  ...
  fromRow: (r) => ({
    ...
    // Billz bergan XOM summa — ishora tuzatilmasdan. `moslik.sale-sign`
    // shuni tekshiradi: tuzatish faqat ALMASHUV chekida kutiladi,
    // boshqa turda ishora teskari bo'lsa — haqiqiy nosozlik.
    totalXom: Number(r.total),
    cash: Number(r.cash), card: Number(r.card), payme: Number(r.payme),
    fromBalance: Number(r.from_balance), debt: Number(r.debt),
    // Billz'da TANILMAGAN to'lov turi bilan kelgan pul. Odatda 0.
    // Noldan katta bo'lsa — `PAYMENT_KINDS` ga yangi tur qo'shish
    // kerak; ungacha bu pul hech qaysi hamyonga tushmaydi va kassa
    // solishtiruvida ko'rinmaydi (`audit.js` → `billz-nomalum-tolov`).
    unknownPaid: Number(r.unknown_paid ?? 0),
    change: 0,
    cashier: r.cashier_id,
    // Billz chekida sotuvchi CHEK QATORI darajasida keladi va
    // `billzMap.mainSeller()` eng ko'p uchraganini tanlaydi. NSPOS
    // profillari bilan bog'lanmagan — shuning uchun ism MATN bo'lib
    // turadi. `cashier_id` esa Billz cheklarida DOIM bo'sh: chek
    // NSPOS kassasidan o'tmagan. Ilgari jadvalda o'sha bo'sh ustun
    // ko'rsatilardi, ya'ni "Abduvohid Kassa" bazada turgani holda
    // ekranda hech kim ko'rinmasdi.
    billzUserId: r.billz_user_id ?? null,
    billzUserName: r.billz_user_name ?? null,
...
// —— Chekni kim sotgan ————————————————————————————————
// Bitta funksiya, chunki savol ikki joyda beriladi (cheklar jadvali va
// chek oynasi) va javob har joyda bir xil bo'lishi kerak.
//
// Uch manba, shu tartibda:
//   1. Billz cheki — sotuvchi ismi chek bilan birga keladi. Billz uni
//      har CHEK QATORIGA yozadi, `billzMap.mainSeller()` esa eng ko'p
//      uchraganini tanlaydi.
//   2. NSPOS kassasidan o'tgan chek — `cashier_id` profilga bog'lanadi.
//   3. Demo/eski yozuv — `cashier` da ism matn bo'lib turadi.
//
// Ilgari faqat 2-yo'l ko'rsatilardi. Billz cheklarida `cashier_id`
// DOIM bo'sh (chek NSPOS kassasidan o'tmagan), ya'ni "Abduvohid Kassa"
// bazada turgani holda ustun butun tarix bo'yicha bo'm-bo'sh edi.
export function sotuvchiNomi(sale, staff = []) {
  if (!sale) return "—";
  if (sale.billzUserName) return sale.billzUserName;
  if (!sale.cashier) return "—";
  return staff.find((x) => x.id === sale.cashier)?.name ?? sale.cashier;
}
