...
import { salesInRange, storeTotalsInRange, qatorTannarx } from "./salesData";
...
import { overallDebtStats, shuKuniYopilgan } from "./debtsData";
...
function billzInRange(from, to) {
  ...
  for (const s of salesInRange(from, to)) {
    ...
    if (!row) { row = { date, storeId: s.storeId, revenue: 0, netRevenue: 0, grossProfit: 0, tannarxsiz: 0 }; rows.set(key, row); }
    ...
    for (const i of s.items ?? []) {
      // BITTA QOIDA — `salesData.qatorTannarx` (xizmat 0, qator, katalog,
      // noma'lum null) — `pnlData.salesPnl` bilan AYNAN bir xil: noma'lum
      // tannarx 0 deb olinmaydi, qator `tannarxsiz` ga tushadi va foyda
      // shu summa bo'yicha "aniq emas" deb belgilanadi (DAFTAR 20).
      const { tannarx } = qatorTannarx(i, katalog.get(i.productId));
      if (tannarx == null) row.tannarxsiz = +(row.tannarxsiz + i.total).toFixed(2);
      row.grossProfit = +(row.grossProfit + (i.total - i.qty * (tannarx ?? 0))).toFixed(2);
    }
  }
...
export function kpis(from, to) {
  ...
  const opex = +(pnl.expenses.opex + pnl.expenses.operationsTotal
    ...
    + pnl.expenses.installerShare + (pnl.expenses.boshqaChiqim ?? 0)).toFixed(2);   // oyliksiz xarajat
  const wages = pnl.expenses.payroll;            // payrollData.payrollCost
  const soliq = pnl.soliq?.summa ?? 0;           // soliq zaxirasi (sozlama, standart 0)
  const netProfit = pnl.netProfit;

  return {
    revenue, grossProfit, soliq,
    ...
    // Shundan SHU KUNIYOQ yopilgani — kassir "keyin to'laydi" uchun ham
    // nasiya bosadi; haqiqiy kredit = onCredit − shuKuniYopilgan (DAFTAR 20 C)
    shuKuniYopilgan: shuKuniYopilgan(from, to).summa,
  };
}
