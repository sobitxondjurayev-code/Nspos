...
//
// Yuklama zaxira bo'lib qoladi: chek qatorlari yo'q davr so'ralsa
// (masalan 39 ta bog'lanmagan eski chek) eski manba ishlatiladi.
function billzInRange(from, to) {
  ...
  for (const s of salesInRange(from, to)) {
    ...
    for (const i of s.items ?? []) {
      sawItems = true;
      row.grossProfit = +(row.grossProfit + (i.total - (i.qty * (i.costPrice ?? 0)))).toFixed(2);
    }
  }

  if (rows.size && sawItems) return [...rows.values()];

  // Zaxira: chek qatorlari yo'q — eski manbadan
  const up = uploadedDaily();
  const src = up.ready ? up.rows : [];
...
export function alerts(from, to, now = new Date()) {
  ...
  const tugagan = cov.filter((c) => c.tugagan);
  if (tugagan.length) {
