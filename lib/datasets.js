...
import { supabase, DEMO_MODE, registerModule, dbXabar, xatoMatni } from "./db";
...
export async function addDataset({ name, reportId, reportLabel, header, rows, sheetName, fileName }) {
  ...
  // Xato — brauzerdagi nusxa qoladi, lekin foydalanuvchi BILADI:
  // ilgari jim o'tardi va fayl "yuklandi" deb turib, boshqa hech kim
  // uni ko'rmasdi (2026-09-03)
  if (error) {
    console.error("[datasets]", error.message);
    dbXabar(`hisobot fayli: ${xatoMatni(error)} — faqat shu brauzerda saqlandi`);
    return local;
  }
  ...
  for (let i = 0; i * CHUNK < rows.length; i++) {
    ...
    if (e2) {
      console.error("[datasets] bo'lak", i, e2.message);
      dbXabar(`hisobot fayli (${i + 1}-bo'lak): ${xatoMatni(e2)} — fayl bazada to'liq emas`);
      break;
    }
  }
...
export async function removeDataset(id) {
  ...
  if (error) dbXabar(`hisobot faylini o'chirish: ${xatoMatni(error)}`);
  else if (!data?.length) dbXabar("hisobot faylini o'chirish: ruxsat yo'q yoki yozuv topilmadi");
}
