...
import { listDebts, remainingOf as debtRemaining } from "./debtsData";
import { payablesSummary } from "./suppliersData";
import { listCustomers } from "./customersData";
import { payrollSummary, listPayrollPayments } from "./payrollData";
...
export function receivables() {
  const open = listDebts()
    .map((d) => ({ ...d, remaining: debtRemaining(d) }))
...
export function payrollLiability(asOf = new Date()) {
  const accrued = payrollSummary(ledgerStartDate(), asOf).total;
  const paid = +listPayrollPayments().reduce((a, p) => a + p.amount, 0).toFixed(2);
