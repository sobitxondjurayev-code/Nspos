...
import { payrollSummary } from "./payrollData";
...
export function expensesPnl(from, to) {
  ...
  const oldPay = +(payroll.fixed + payroll.salesBonus + payroll.planBonus).toFixed(2);
  // KPI bor bo'lsa ish haqi manbai o'sha — eski hisob takrorlanmaydi
  const payrollCost = kpiPay > 0 ? kpiPay : oldPay;
  if (payrollCost > 0) byCategory["Ish haqi"] = +((byCategory["Ish haqi"] ?? 0) + payrollCost).toFixed(2);
  ...
  return {
    ...
    total: +(operationsTotal + opexTotal + payrollCost + writeoff + shrinkage).toFixed(2),
  };
}
...
export function billzCashflow(from, to) {
  ...
  const rows = EXPORT_CASHFLOW_DAILY.filter(
    (r) => !from || (r.date >= String(from).slice(0, 10) && r.date <= String(to).slice(0, 10))
  );
