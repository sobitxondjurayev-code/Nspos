...
export function salesPnl(from, to) {
  ...
  if (itemsBor) {
    let revenue = 0, cogs = 0, returns = 0, discounts = 0;
    for (const s of rows) {
      ...
      for (const i of s.items ?? []) {
        // Tannarx chek qatorining O'ZIDA saqlangan (sotilgan paytdagi).
        // Bo'lmasa katalogdagi joriy tannarx — 34 489 qatordan
        // 806 tasida shunday (2.3 %).
        const unit = i.costPrice || catalog.find((x) => x.id === i.productId)?.costPrice || 0;
