...
const invoiceSync = syncTable("supplier_invoices", {
  ...
  fromRow: (r) => ({
    id: r.id,
    no: r.source === "billz" ? "XB-" + String(r.billz_id ?? r.id).slice(0, 6) : "HF-" + String(r.id).slice(0, 6),
    ...
    // Billz hujjatida muddat bo'lmasa — hujjat sanasi (kechikish 0 dan)
    dueDate: r.due_date ?? r.invoice_date,
    note: r.note ?? "",
    // Billz ko'zgusi (DAFTAR 20 E): to'langani Billz'niki, qayta hisoblanmaydi
    source: r.source ?? "nspos",
    billzId: r.billz_id ?? null,
    paidAmount: Number(r.paid_amount ?? 0),
    supplyTotal: r.supply_total == null ? null : Number(r.supply_total),
    retailTotal: r.retail_total == null ? null : Number(r.retail_total),
...
// Billz hujjatida to'langan summa Billz'ning o'zidan (`paid_amount`);
// NSPOS fakturasida to'lov yozuvlaridan — qarz qoldig'i kabi (DAFTAR 17).
export const paidOf = (inv) => inv.source === "billz"
  ? +(inv.paidAmount ?? 0).toFixed(2)
  : +inv.payments.reduce((a, p) => a + p.amount, 0).toFixed(2);
