...
import { supabase, DEMO_MODE } from "./db";
...
function buildSeedOrders() {
  ...
  for (let back = 190; back >= 0; back -= 1 + Math.floor(rnd() * 4)) {
    ...
    out.push({
      ...
      storeId: installer.storeId ?? demoStores[0].id,
...
async function saveOrderItems(o) {
  if (DEMO_MODE) return;
  await supabase.from("service_items").delete().eq("order_id", o.id);
  const rows = [
    ...(o.services ?? []).map((x) => ({
  ...
  if (rows.length) await supabase.from("service_items").insert(rows);
}
...
export function addOrder(data) {
  ...
  orders = [item, ...orders];
  orderSync.created(item).then(() => saveOrderItems(getOrder(item.id) ?? item));
  return item;
}
