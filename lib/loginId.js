...
// Kirish uchun login/parolni bazadagi "email" ko'rinishiga o'giradi.
//
// Uch xil login qabul qilinadi:
//
//   "90 123 45 67"          → 998901234567@nspos.app   (telefon)
//   "abdulvosit@nspos.app"  → o'zi                     (to'liq email)
//   "Abdulvosit"            → abdulvosit@nspos.app     (nom)
//
// Uchinchisi 2026-08-22 da qo'shildi. Ilgari nom bilan kirib
// bo'lmasdi: `phoneToEmail` harflarni tashlab yuborardi va login
// jimgina "@nspos.app" ga aylanardi — hisob bazada bo'lsa ham hech
// qachon topilmasdi va "parol noto'g'ri" deb turardi.
//
// Telefonli hisoblarga ta'siri yo'q: raqamda harf bo'lmaydi.
export function toCredential(idRaw, password) {
  const id = String(idRaw ?? "").trim();
  if (id.includes("@")) return { email: id.toLowerCase(), password };
  if (/[a-zA-Z]/.test(id)) return { email: `${id.toLowerCase()}@nspos.app`, password };
