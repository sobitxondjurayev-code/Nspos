...
export function moneyWarnings(user) {
  ...
  if (isOwner) {
    ...
    if (bosh.length) {
      const ulush = Math.round((bosh.length / Math.max(1, sales.length)) * 100);
      out.push({
        id: "billz-items-missing", level: bosh.length === sales.length ? "error" : "warn",
        title: `${bosh.length} ta chekda tovar tarkibi yo'q (${ulush}%)`,
        detail: "Tovar kesimidagi foyda, ABC tahlil va \"eng ko'p sotilgan\" shu qatorlardan "
          + "hisoblanadi. Sozlamalar → Billz'dan yangilash orqali tortiladi; "
          + "butun tarix uchun `npm run billz -- --only=orders --full`.",
