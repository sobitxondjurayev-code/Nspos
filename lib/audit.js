...
export function moneyWarnings(user) {
  ...
  // Kassa kirimi (`kassaIncome.kunlikKirim`) faqat naqd va Payme'ni
  // sanaydi; karta (Uzcard/Humo/Click), mijoz balansidan va tanilmagan
  // to'lov chek summasida bor-u kassaga KIRMAYDI. 02.09 da uchalasi
  // 0.00 (o'lchandi) — birinchi qizarish haqiqiy bo'ladi: pul jimgina
  // yo'qolmasin, hamyon qo'shilsin.
  if (isOwner) {
    const boshi = new Date(getLedgerStart() + "T00:00:00").getTime();
    let karta = 0, balans = 0, nomalum = 0;
    for (const s of listSales()) {
      if (new Date(s.at).getTime() < boshi) continue;
      karta += +s.card || 0; balans += +s.fromBalance || 0; nomalum += +s.unknownPaid || 0;
    }
    const jami = +(karta + balans + nomalum).toFixed(2);
    if (jami > 0) {
      out.push({
        id: "kirim-hamyonsiz", level: "warn",
        title: `${money(jami)} kassa ko'rmaydigan hamyonga tushgan`,
        detail: `Karta ${money(karta)}, mijoz balansidan ${money(balans)}, tanilmagan ${money(nomalum)} `
          + `(${getLedgerStart()} dan). Kassa kirimi faqat naqd va Payme'ni sanaydi — bu pul `
