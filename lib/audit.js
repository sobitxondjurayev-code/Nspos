...
export function moneyWarnings(user) {
  ...
  // maydon qo'shilsa jamisi va tekshiruvi ham o'sha zahoti).
  {
    ...
    const MANBALAR = [
      ...
      { id: "kpi", nom: "KPI kunligi", ogoh: 2, xato: 4,
        oxirgi: () => engOxirgi(listAllDays().map((d) => d.date)),
  ...
  // ══════════════════════════════════════════════════════════════
  // N+1) EXCEL YUKLAMASI ESKIRGAN
  // ══════════════════════════════════════════════════════════════
  // Moliya sahifalari va rahbariyat paneli qismlarini hamon QO'LDA
  // yuklanadigan Billz Excel'idan oladi (`components/useUploadRows`).
  // Ular bir marta yuklanadi va o'z-o'zidan yangilanmaydi — 24-avgustda
  // eng yangisi 16-avgust, eng eskisi 5-avgust edi. Ekranda esa hech
  // qanday belgi yo'q: raqam turibdi, demak "ishlayapti" ko'rinadi.
  //
  // Sotuv va qarz Billz API'sidan keladi va yangi; aynan shu farq
  // xatoni yashiradi — bir sahifada yangi raqam ham, uch haftalik
  // raqam ham yonma-yon turadi.
  if (isOwner) {
    const bugun = iso(new Date());
    // Faqat sahifalar HAQIQATDAN o'qiydigan hisobotlar. Ro'yxat
    // `useUploadRows([...])` chaqiruvlaridan olingan.
    const KERAK = {
      summary: "Kunlik yakun", cashflow: "Pul oqimi (ДДС)",
      efficiency: "Tovar samaradorligi", client_debts: "Mijozlar qarzi",
      pnl: "Foyda va zarar",
    };
    const OGOH = 3, XATO = 7;

    const oxirgisi = new Map();
    for (const d of listDatasets()) {
      const kun = iso(d.at);
      if (!KERAK[d.reportId] || !kun) continue;
      const bor = oxirgisi.get(d.reportId);
      if (!bor || kun > bor) oxirgisi.set(d.reportId, kun);
    }

    const eski = [];
    for (const [id, nom] of Object.entries(KERAK)) {
      const kun = oxirgisi.get(id);
      // Umuman yuklanmagani ham aytiladi: o'sha sahifa bo'sh turadi
      const yosh = kun ? Math.floor((new Date(bugun) - new Date(kun)) / 86400000) : null;
      if (yosh === null) eski.push({ nom, matn: "umuman yuklanmagan", yosh: 999 });
      else if (yosh >= OGOH) eski.push({ nom, matn: `${kun} · ${yosh} kun`, yosh });
    }

    if (eski.length) {
      eski.sort((a, b) => b.yosh - a.yosh);
      out.push({
        id: "stale-uploads", level: eski.some((e) => e.yosh >= XATO) ? "error" : "warn",
        title: `${eski.length} ta Billz hisoboti eskirgan`,
        detail: eski.map((e) => `${e.nom} — ${e.matn}`).join(" · ")
          + ". Moliya sahifalari va rahbariyat paneli shu fayllardan o'qiydi, "
          + "ya'ni ularda o'sha kundan keyingi savdo KO'RINMAYDI (sotuv va qarz "
          + "esa Billz API'sidan yangi kelib turadi — bir ekranda ikki xil davr). "
          + "Billz'dan qaytadan eksport qilib, Ma'lumot bo'limiga tashlang.",
        action: "Ma'lumot bo'limini ochish", href: "/data",
        // Yuqoridagi bilan bir xil sabab: faylni ODAM yuklaydi,
        // kod bilan tuzatib bo'lmaydi.
        bloklamaydi: true,
      });
    }
  }
