...
import { allNps } from "./npsData";
import { listStaff } from "./staffData";
import { expensesInRange, listOneOff, STREET_INSTALLER, listExpenseCategories } from "./expensesData";
import { lastRate } from "./ratesData";
import { ymd } from "./dates";
import { getUsdRate, getLedgerStart, sozlama } from "./companyData";
...
import { salesPnl } from "./pnlData";
import { listMuhrlar, muhr, otganOy, muhrFarqi } from "./oyMuhri";
import { listInvoices } from "./suppliersData";
...
export function moneyWarnings(user) {
  ...
  // N.4b) Kassa ko'rmaydigan hamyonga pul tushgan.
  // Kassa kirimi (`kassaIncome.kunlikKirim`) naqd va bank (Payme +
  // karta) hamyonini sanaydi; mijoz balansidan va tanilgan to'lov
  // chek summasida bor-u kassaga KIRMAYDI. 02.09 da ikkalasi 0.00
  // (o'lchandi) — birinchi qizarish haqiqiy bo'ladi: pul jimgina
  // yo'qolmasin, hamyon qo'shilsin. Karta 2026-09-04 dan bank
  // hamyonida (DAFTAR 20) — bu ro'yxatdan chiqdi.
  if (isOwner) {
    const boshi = new Date(getLedgerStart() + "T00:00:00").getTime();
    let balans = 0, nomalum = 0;
    for (const s of listSales()) {
      if (new Date(s.at).getTime() < boshi) continue;
      balans += +s.fromBalance || 0; nomalum += +s.unknownPaid || 0;
    }
    const jami = +(balans + nomalum).toFixed(2);
    if (jami > 0) {
      out.push({
        id: "kirim-hamyonsiz", level: "warn",
        title: `${money(jami)} kassa ko'rmaydigan hamyonga tushgan`,
        detail: `Mijoz balansidan ${money(balans)}, tanilmagan ${money(nomalum)} `
          + `(${getLedgerStart()} dan). Kassa kirimi naqd va bank (Payme/karta) hamyonini sanaydi — bu pul `
          + "hech qaysi hamyonda ko'rinmaydi. `lib/kassaIncome.js` ga hamyon qo'shilsin.",
        action: "Sotuvlarni ochish", href: "/sales",
      });
    }
  }

  // N.4c) Tannarxi NOMA'LUM sotuv — foydasi hisoblanmagan.
  // `salesData.qatorTannarx` 0/null tannarxni "bepul" demaydi (100 %
  // foyda chiqardi) — null qaytaradi va P&L uni tannarxga QO'SHMAYDI.
  // Bu qatorlar yalpi foydada to'liq tushum bo'lib turadi, ya'ni foyda
  // shu summa bo'yicha ANIQ EMAS. ODAM kiritadi (Billz'da kirim narxi)
  // — bloklamaydi. 04.09 da avgustda 2 qator, 230.86 $ (o'lchandi).
  if (isOwner) {
    const oyBoshi = new Date(); oyBoshi.setDate(1); oyBoshi.setHours(0, 0, 0, 0);
    const sp = salesPnl(oyBoshi, new Date());
    if (sp.tannarxsiz?.summa > 500) {
      out.push({
        id: "tannarxsiz-sotuv", level: "warn",
        title: `Shu oyda ${sp.tannarxsiz.qatorlar} ta chek qatori tannarxsiz — ${money(sp.tannarxsiz.summa)} sotuvning foydasi noma'lum`,
        detail: "Bu qatorlar P&L tannarxiga kirmagan (0 deb yozilmaydi — 0 «bepul» degani). "
          + "Billz'da tovarning kirim narxini kiriting; keyingi sinxronda tannarx keladi.",
  ...
  // N.4d) Xarajat turi bir vaqtda "Servis" ham, "Tannarxga" ham (2026-09-05).
  //
  // Servis turini `components/ServiceReport.jsx` xizmat tannarxiga
  // qo'shadi, `cogs` turini esa P&L tovar tannarxiga — ikkalasi
  // yoqilgan bo'lsa BIR PUL IKKI MARTA sanaladi. Bazada `check`
  // cheklovi bor (`expcat_cogs_service`), bu — ikkinchi qulf: eski
  // qator yoki qo'lda `update` bilan holat baribir buzilishi mumkin.
  // Hozir bunday tur 0 ta, ya'ni birinchi qizarish HAQIQIY bo'ladi.
  {
    const ziddiyat = listExpenseCategories().filter((c) => c.service && c.cogs);
    if (ziddiyat.length) {
      out.push({
        id: "kelish-service", level: "error",
        title: `${ziddiyat.length} ta xarajat turi bir vaqtda "Servis" ham, "Tannarxga" ham: ${ziddiyat.map((c) => c.label).join(", ")}`,
        detail: "Servis turi \"Servis foydasi\"da xizmat tannarxiga, \"Tannarxga\" esa P&L tovar tannarxiga qo'shiladi — "
          + "ikkalasi yoqilsa bir xarajat ikki marta hisoblanadi. Sozlamalar → Xarajat turlarida bittasini o'chiring.",
        action: "Sozlamalarni ochish", href: "/settings",
      });
    }
  }

  // N.4f) Yo'lkira Xarajatlarda emas, kassa chiqimida yozilgan (2026-09-05).
  //
  // `kassa_ops` dagi `goods`/`import`/`other_out` chiqimlari P&L ga
  // UMUMAN kirmaydi (ombor aktivi deb hisoblanadi). Ya'ni izohida
  // "yo'lkira" deb yozilgan pul na xarajatda, na tannarxda ko'rinadi —
  // u foyda hisobidan butunlay tushib qoladi. Endi buning to'g'ri
  // joyi bor: Xarajatlar → "Import yo'lkira" / "Dastavka".
  //
  // 05.09 o'lchov: aynan 2 qator mos keladi (4 341.00 "Kabel yolkirasi"
  // va 1 977.62 "Import yo'lkira"), ikkalasi ham haqiqiy — 0 ta yolg'on
  // moslik. ODAM qiladigan ish, shuning uchun bloklamaydi.
  if (isOwner) {
    const chegara = Number(sozlama("tannarx.kelishOgohChegara", 100)) || 100;
    const naqsh = /yo.?l ?kira|yul ?kira|dostavka|dastavka|freight|yetkaz/i;
    const b = new Date(); const a = new Date(b.getTime() - 90 * 864e5);
    const ymdA = iso(a), ymdB = iso(b);
    const qatorlar = listOps().filter((o) => o.kind === "out"
      && ["goods", "import", "other_out"].includes(o.category)
      && o.date >= ymdA && o.date <= ymdB && naqsh.test(o.note ?? ""));
    const summa = +qatorlar.reduce((s2, o) => s2 + o.amount, 0).toFixed(2);
    if (summa >= chegara) {
      out.push({
        id: "kelish-kassa-ops", level: "warn",
        title: `${money(summa)} (${qatorlar.length} yozuv) tovar kelish xarajati kassa chiqimida turibdi`,
        detail: qatorlar.slice(0, 3).map((o) => `${o.date} · ${o.note} · ${money(o.amount)}`).join(" · ")
          + ". Bu pul P&L ga umuman kirmaydi — na xarajatda, na tannarxda ko'rinadi. "
          + "Xarajatlarga \"Import yo'lkira\" yoki \"Dastavka — tovar kelishi\" turi bilan kiriting, "
          + "shunda u tannarxga qo'shiladi va yalpi marja haqiqiy bo'ladi.",
        action: "Kassa harakatini ochish", href: "/finance/operations",
        bloklamaydi: true,
      });
    }
  }

  // N.4e) Tovar xaridi hujjati yuritilmayapti (DAFTAR 20 E).
  // Oxirgi 30 kunda sotilgan tovar tannarxi bilan YOZILGAN xarid (Billz
  // supplier-order + Pul rejasidagi tovar/import to'lovi) solishtiriladi.
  // Ombor barqaror bo'lsa xarid ≈ tannarx; yozilgani YARMIDAN kam bo'lsa —
  // ombor kirimi hech qayerda yozilmayapti: balansda ta'minotchi qarzi 0
  // turadi, ombor harakati yopilmaydi. ODAM qiladigan ish (Billz'da
  // "Приход") — bloklamaydi. 05.09 o'lchov: 30 kun tannarx ~91 000 $,
  // Billz xarid hujjati 0, Pul rejasi tovar to'lovi ~37 600 $ (41 %) —
  // birinchi qizarish haqiqiy.
  if (isOwner) {
    const b = new Date(); const a = new Date(b.getTime() - 30 * 864e5);
    const tannarx = salesPnl(a, b).cogs;
    const ymdA = iso(a), ymdB = iso(b);
    const billzXarid = +listInvoices().filter((i) => i.source === "billz" && i.invoiceDate >= ymdA && i.invoiceDate <= ymdB)
      .reduce((s, i) => s + (i.supplyTotal ?? i.amount), 0).toFixed(2);
    const tolov = +listOps().filter((o) => o.kind === "out" && ["goods", "import"].includes(o.category) && o.date >= ymdA && o.date <= ymdB)
      .reduce((s, o) => s + o.amount, 0).toFixed(2);
    if (tannarx > 1000 && billzXarid + tolov < tannarx / 2) {
      out.push({
        id: "xarid-yozilmagan", level: "warn",
        title: `30 kunda tannarx ${money(tannarx)}, yozilgan xarid ${money(billzXarid + tolov)} — tovar kirimi hujjatsiz`,
        detail: `Billz xarid hujjati (supplier-order) ${money(billzXarid)}, Pul rejasi tovar/import to'lovi ${money(tolov)}. `
          + "Ombor kirimi yozilmasa ta'minotchi qarzi 0 turadi va \"pul qayerga ketdi\" javobsiz qoladi. "
          + "Billz'da xaridni «Заказ поставщику / Приход» bilan yuriting — ko'zgu tayyor.",
  ...
  // N.4d) Oy yopilmagan / muhr o'zgargan (DAFTAR 20 K).
  // O'tgan oy 5-sanagacha yopilmasa — ODAM qilmagan ish (bloklamaydi).
  // Yopilgan oyning hozirgi hisobi muhrdan farq qilsa — muhr yashirin
  // o'zgarmaydi, farq shu yerda: kech kiritilgan xarajat, Billz'da
  // tuzatilgan chek yoki kurs.
  if (isOwner) {
    const oy = otganOy(new Date());
    if (!muhr(oy) && new Date().getDate() >= 5) {
      out.push({
        id: "oy-yopilmagan", level: "warn",
        title: `${oy} oyi hali yopilmagan`,
        detail: "Balans → \"Oy muhri\" → oyni yopish: P&L, qarz, kassa, ombor, kapital muhrlanadi. Yopilmagan oy raqami o'zgarib turaveradi.",
        action: "Balansni ochish", href: "/finance/balance",
        bloklamaydi: true,
      });
    }
    for (const m of listMuhrlar().slice(0, 3)) {
      const f = muhrFarqi(m);
      if (!f.bor) continue;
      out.push({
        id: `oy-muhri-${m.oy}`, level: "warn",
        title: `${m.oy} muhrlangandan keyin hisob o'zgargan: foyda ${money(f.sofFoyda)}, qarz ${money(f.qarz)}, kassa ${money(f.kassa)}`,
        detail: `Muhr: foyda ${money(m.sofFoyda)}, qarz ${money(m.qarz)}, kassa ${money(m.kassa)}. Sabab — kech kiritilgan xarajat, Billz'da tuzatilgan chek yoki kurs. Muhr o'zgarmaydi; farq keyingi oyga "tuzatish" bo'lib o'tadi.`,
        action: "Balansni ochish", href: "/finance/balance",
        bloklamaydi: true,
      });
    }
  ...
  // maydon qo'shilsa jamisi va tekshiruvi ham o'sha zahoti).
  {
    ...
    const MANBALAR = [
      ...
      // NPS — retention menejer QO'LDA kiritadi. 2026-09-03 da butun
      // bazada 1 ta baho bor edi (iyulniki), ya'ni "Ustalar
      // reytingi"dagi NPS ustuni hamma uchun "—" turardi va buni
      // hech narsa aytmasdi. Chegara uzunroq: baho har kuni emas,
      // o'rnatishdan keyin qo'ng'iroq qilinganda qo'yiladi.
      { id: "nps", nom: "NPS baholari", ogoh: 7, xato: 14,
        oxirgi: () => engOxirgi(allNps().map((r) => r.installedDate || iso(r.createdAt))),
        amal: "NPS ni ochish", href: "/nps",
        izoh: "Baho kiritilmasa ustalar reytingidagi NPS ustuni bo'sh turadi." },
