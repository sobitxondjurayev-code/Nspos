...
//   3. 403 ("access denied") ni ROL bilan ochib bo'lmaydi. Integratsiya
//      kalitida QAT'IY RO'YXAT (whitelist) bor va u kabinetdan
//      boshqarilmaydi — 2026-09-06 da o'lchandi (DAFTAR 24): rolda
//      «Списание» ning hamma huquqi yoqilgan bo'lsa ham `/v2/write-off`
//      403 beradi, «Поставщики» ning huquqi butunlay o'chiq bo'lsa ham
//      `/v1/supplier` ochiq. Yopiq metodni faqat Billz o'z tomonidan
//      ocha oladi. Bo'sh ro'yxat esa boshqa narsa — parametr xatosi.
...
/**
 * Transferlar (sklad ↔ filial ko'chirish). Sinov (2026-09-03, DAFTAR 18):
 * `/v2/transfer` — bor, 2 063 yozuv, yangisidan eskisiga; sana filtri
 * YO'Q (`start_date` 400, `from_date` e'tiborsiz); `/v2/transfer/{id}`
 * sarlavhani qaytaradi, `transfer_items` doim null; qator yo'llari
 * (`transfer-item`, `/items`, `/products`…) 404. Ya'ni faqat sarlavha:
 * kimdan-kimga, dona, tannarx/sotuv summasi, kim, qachon.
 */
export const transferPages = (opts) =>
  pages("/v2/transfer", {}, (b) => b.transfer, opts);
...
/**
 * Muddati o'tganlar — `status=overdue`. DIQQAT: `unpaid` filtri bulardan
 * faqat TO'LOV TUSHMAGANINI beradi (02.09 da 277/374); qisman
 * to'langan 96 tasi faqat shu yerda keladi. Ilgari yengil rejim faqat
 * `unpaid` ni olardi va qisman to'lov 20 soatlik to'liq yurishgacha
 * ko'rinmasdi. Qabul qilinadigan `status` qiymatlari: `unpaid`,
 * `overdue` — boshqasi ("partial_paid", "fully_paid") 400 beradi.
 */
export const overdueDebtPages = (opts) =>
  pages("/v1/debt", { status: "overdue" }, (b) => b.data, opts);

/**
 * Bitta mijozning BARCHA qarzlari — yopilganlari ham. Bazada ochiq-u
 * yuqoridagi oqimlarda kelmagan qarz Billz'da yopilgan bo'ladi; uni
 * id bo'yicha so'rab bo'lmaydi (`/v1/debt/{id}` → 403), mijoz bo'yicha
 * bo'ladi (`customer_id`). Shu bilan yopilish 5 daqiqada ko'rinadi.
 */
export const customerDebtPages = (billzCustomerId, opts) =>
  pages("/v1/debt", { customer_id: billzCustomerId }, (b) => b.data, opts);

/** Bitta chek — ichidagi tovarlar bilan. Asosiy teshikni shu yopadi. */
export const fetchOrder = (id) => billzGet(`/v2/order/${id}`);

/**
 * Xarid hujjatlari (Billz "Заказ поставщику / Приход") — `/v2/supplier-order`.
 * 2026-09-05 probe: ochiq, javob `{ count, supplier_orders[] }`; qatorlari
 * (`/v2/supplier-order/{id}`) 403. Kompaniyada butun tarixda 1 ta hujjat —
 * modul ishlatilmaydi; ko'zgu tayyor tursin (DAFTAR 20 E).
 */
export const supplierOrderPages = (opts) =>
  pages("/v2/supplier-order", {}, (b) => b.supplier_orders, opts);

/**
 * Inventarizatsiya (Billz "Инвентаризация") — `/v2/stocktaking`.
 * 2026-09-06 o'lchov (DAFTAR 24): OCHIQ, javob `{ count, stocktakings[] }`.
 * Yo'l ochiq hujjatda yo'q — kabinet kuzatilib topilgan; ilgari 45 nomzod
 * urilib "metod yo'q" degan xulosa chiqarilgandi.
 *
 * Sarlavha darajasida: `items` ro'yxatda doim `null`, `/v2/stocktaking/{id}`
 * esa yopiq. Ya'ni "qaysi tovar" emas, "qancha kamomad/ortiqcha va qancha
 * summa" savoliga javob bor.
 */
export const stocktakingPages = (opts) =>
  pages("/v2/stocktaking", {}, (b) => b.stocktakings, opts);

// —— Tashxis ————————————————————————————————————
// "Nega sotuvlar kelmayapti?" degan savolga bir chaqiriqda javob:
// qaysi metod ochiq, qaysisi huquq kutyapti.
// `kengaytirilgan` — Billz'da TRANSFER (sklad → filial ko'chirish) API
// bor-yo'qligini sinash (2026-09-03, rahbar: "skladdan qancha transfer
// bo'layotgani ko'rinmayapti"). Ochiq hujjatda bunday metod yo'q —
// nomzodlar birma-bir uriladi, natija DAFTAR'ga yoziladi. Oddiy
// tashxisda (Sozlamalar, cron) chaqirilmaydi — sekinlashtirmasin.
export async function probe({ kengaytirilgan = false } = {}) {
  const checks = [
    ...
    // DAFTAR 24 (2026-09-06): yo'llar kabinetning O'ZI chaqirgan
    // manzillardan olindi — ilgari ular taxmin bilan qidirilgandi va
    // ikkitasi noto'g'ri edi (`/v1/order/cash-shifts` → aslida
    // `/v1/cashbox-shifts`; inventarizatsiya "yo'q" deb hisoblanardi —
    // aslida `/v2/stocktaking` va u OCHIQ).
    ["supplier-orders (xarid)",  () => billzGet("/v2/supplier-order", { limit: 1, page: 1 }, { retries: 0 })],
    ["stocktaking (inventar.)",  () => billzGet("/v2/stocktaking", { limit: 1, page: 1 }, { retries: 0 })],
    ["write-off (spisanie)",     () => billzGet("/v2/write-off", { limit: 1, page: 1 }, { retries: 0 })],
    ["cashbox-shifts (smena)",   () => billzGet("/v1/cashbox-shifts", { limit: 1, page: 1 }, { retries: 0 })],
    ["gl-transaction (kassa)",   () => billzGet("/v1/gl-transaction", { limit: 1, page: 1 }, { retries: 0 })],
  ];
  if (kengaytirilgan) {
    for (const yol of ["/v1/transfer", "/v2/transfer", "/v1/transfers", "/v2/transfers",
                       "/v1/product-transfer", "/v1/stock-transfer", "/v1/movement",
                       "/v2/movements", "/v1/stock-movement", "/v2/stock", "/v1/warehouse-operation"]) {
      checks.push([`transfer:${yol}`, () => billzGet(yol, { limit: 1, page: 1 }, { retries: 0 })]);
    }
    checks.push(["transfer:/v1/order?type=transfer", () => billzGet("/v1/order", { type: "transfer", limit: 1 }, { retries: 0 })]);
  }
  const out = {};
