...
export async function probe({ kengaytirilgan = false } = {}) {
  const checks = [
    ...
    // DAFTAR 20 (E, F): xarid ochiq; spisanie va kassa smenasi BOR, lekin
    // kalit roliga huquq berilmagan (403) — Billz → Sozlamalar →
    // Integratsiya → rolga "Списания" va "Кассовые смены" o'qish huquqi.
    ...
    ["cash-shifts (smena)",     () => billzGet("/v1/order/cash-shifts", { limit: 1, page: 1 }, { retries: 0 })],
  ];
  if (kengaytirilgan) {
