 ...
 * @returns {{ write: object[], same: number }}
 */
...
export async function syncProducts(db, ctx, { since = null } = {}) {
  ...
  const existing = await readAll(db, "products",
    ...
    "category_id,sale_price,cost_price,is_variative,is_active");
  ...
  const PRODUCT_FIELDS = ["name", "sku", "barcode", "category_id", "brand",
                          ...
                          "description", "is_variative", "is_active"];
  ...
  return {
    fetched, inserted: pChanged.write.length, updated: relinked,
...
export async function syncCustomers(db, ctx, { since = null, full = false } = {}) {
  ...
  return {
    fetched, inserted: cChanged.write.length, updated: 0, stopped,
...
export async function syncDebts(db, ctx, { deadline = Infinity, mode = "full", since = null } = {}) {
  ...
  const haveDebts = new Map(
    (await readAll(db, "debts", "id,billz_id,amount,paid_amount,status,closed_at,due_date,customer_id,sale_id,store_id",
  ...
  const DEBT_FIELDS = ["amount", "paid_amount", "status", "closed_at",
                       "due_date", "customer_id", "sale_id", "store_id"];

  async function flush() {
    if (!batch.length) return;

    const { write, same } = onlyChanged(
