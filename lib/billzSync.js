...
import {
  ...
  customerDebtPages, transferPages, billzGet, fmtBillzDay, BillzError, supplierOrderPages,
  stocktakingPages,
} from "./billzApi";
import {
  matchStores, flattenCategories, categoryRow, supplierRow, supplierOrderRow,
  productRow, stockRows, customerRow, saleRow, saleItemRows,
  mergeProduct, mergeCustomer, phoneKey, billzTime, isDeleted, saleType,
  debtRow, debtPaymentRows, servisNomi, transferRow, stocktakingRow,
} from "./billzMap";

export const STAGES = ["categories", "suppliers", "products", "customers", "orders", "debts", "transfers", "supplierOrders", "stocktakings"];
...
export async function buildContext(db) {
  ...
  const [stores, shops] = await Promise.all([
    readAll(db, "stores", "id,name,billz_names"),
    fetchShops(),
  ]);
...
export async function moslikTekshir(db, ctx, { debts = null } = {}) {
  ...
  const dbIds = new Set((await readAll(db, "sales", "billz_id",
    ...
    .map((s) => s.billz_id));
  // 5. Do'kon × kun kesimida SAVDO SUMMASI — o'sha aylanishda yig'iladi
  //    (Billz sekundiga 2 so'rov beradi, ikkinchi marta aylanib
  //    bo'lmaydi).
  //
  //    NEGA KERAK. Yuqoridagi 4-tekshiruv chek BOR-YO'QLIGINI ko'radi,
  //    ya'ni chek bazada bor-u NOTO'G'RI do'konga, noto'g'ri sanaga
  //    yoki noto'g'ri summa bilan yozilgan bo'lsa — jim o'tadi.
  //    Rahbar 2026-09-03 da aynan shunday farqni O'ZI topdi:
  //    "Namanganda 2 099.64 turibdi, bunda 2 067 turibdi". Ya'ni
  //    ekrandagi raqam Billz bilan tenglashmasdi va buni hech narsa
  //    aytmasdi ("Billz — yagona manba", CLAUDE.md 2026-09-03).
  const billzKun = new Map();     // "store|kun" → summa
  const kunKalit = (storeId, soldAt) => `${storeId}|${String(soldAt).slice(0, 10)}`;
  ...
  for await (const o of orderPages({ from, to: new Date() })) {
    ...
    const { row } = saleRow(o, ctx);
    if (!row.store_id || !row.sold_at) continue;
    const k = kunKalit(row.store_id, row.sold_at);
    billzKun.set(k, (billzKun.get(k) ?? 0) + Number(row.total ?? 0));
  }
  ...
  // Bazadagi o'sha oyna. `superseded_by` — almashtirilgan chek, u
  // ikkinchi marta sanalmasligi kerak (`sales-superseded.sql`).
  const nsposKun = new Map();
  for (const s of await readAll(db, "sales", "store_id,sold_at,total",
    (q) => q.not("billz_id", "is", null).is("superseded_by", null)
            .gte("sold_at", from.toISOString()))) {
    if (!s.store_id || !s.sold_at) continue;
    const k = kunKalit(s.store_id, s.sold_at);
    nsposKun.set(k, (nsposKun.get(k) ?? 0) + Number(s.total ?? 0));
  }

  // ENG ESKI KUN SOLISHTIRILMAYDI. Billz KUN bo'yicha filtrlaydi
  // (`fmtBillzDay`), baza esa aniq VAQT bo'yicha — oynaning chegara
  // kuni bir tomonda to'liq, ikkinchisida yarim bo'lib qoladi va
  // "Billz 3 256.18, bu yerda 0" degan SOXTA farq chiqadi (2026-09-04
  // da birinchi yurishda aynan shunday bo'ldi: 27.08 ikkala do'kon
  // uchun ham). Shuning uchun chegara bir kun ichkariga suriladi.
  const chegaraKun = new Date(from.getTime() + 864e5).toISOString().slice(0, 10);

  // Chegara 0.02 $: Billz kasr tiyin bilan beradi (CLAUDE.md
  // 2026-09-03), yaxlitlash farqi shu doirada.
  const kunFarq = [];
  for (const k of new Set([...billzKun.keys(), ...nsposKun.keys()])) {
    if (k.split("|")[1] < chegaraKun) continue;
    const b2 = Math.round((billzKun.get(k) ?? 0) * 100) / 100;
    const n2 = Math.round((nsposKun.get(k) ?? 0) * 100) / 100;
    if (Math.abs(b2 - n2) <= 0.02) continue;
    const [store, kun] = k.split("|");
    kunFarq.push({ store, kun, billz: b2, nspos: n2, farq: Math.round((b2 - n2) * 100) / 100 });
  }
  kunFarq.sort((a, b) => Math.abs(b.farq) - Math.abs(a.farq));
  olchov["7 kun savdo (do'kon × kun)"] = {
    kunlar: billzKun.size, farqli: kunFarq.length,
    // Eng katta beshtasi — jurnal cheksiz o'smasin
    eng: kunFarq.slice(0, 5),
  };
  // Har farqli kun ALOHIDA qator bo'lib tushadi — audit matni
  // "Billz X, bu yerda Y" shaklida chiqadi va qaysi kun ekani
  // darrov ko'rinadi. Faqat eng kattasi beshtasi: ro'yxat uzun
  // bo'lsa ogohlantirish o'qilmay qoladi.
  for (const d of kunFarq.slice(0, 5)) {
    farq.push({ nima: `savdo ${d.kun}`, billz: d.billz, nspos: d.nspos });
  }
  if (kunFarq.length > 5) {
    farq.push({ nima: "savdo — yana farqli kun", billz: kunFarq.length - 5, nspos: 0 });
  }
...
// ══════════════════════════════════════════════════════════════
// XARID HUJJATLARI — Billz supplier-order → supplier_invoices (DAFTAR 20 E)
// ══════════════════════════════════════════════════════════════
// Balansdagi ta'minotchi qarzi (`suppliersData.payablesSummary`) shu
// jadvaldan. Kompaniya Billz xarid modulini hali ishlatmaydi (1 hujjat) —
// ko'zgu tayyor turadi, audit `xarid-yozilmagan` esa tannarx bilan
// yozilgan xaridni solishtirib "hujjat yuritilmayapti" deb aytadi.
export async function syncSupplierOrders(db, ctx, { full = false } = {}) {
  const FIELDS = ["supplier_id", "supplier_name", "store_id", "amount", "supply_total", "retail_total",
                  "qty", "paid_amount", "status_id", "invoice_date", "due_date", "note", "accepted_at", "source"];
  const suppliers = await readAll(db, "suppliers", "id,billz_id", (q) => q.not("billz_id", "is", null));
  ctx.supplierByBillz = new Map(suppliers.map((r) => [r.billz_id, r.id]));
  const existing = await readAll(db, "supplier_invoices",
    "id,billz_id," + FIELDS.join(","), (q) => q.not("billz_id", "is", null));
  const have = new Map(existing.map((r) => [r.billz_id, r]));

  const rows = [];
  let fetched = 0, sahifadaYangi = 0;
  for await (const o of supplierOrderPages({ limit: 100, max: full ? Infinity : 300 })) {
    fetched++;
    const raw = supplierOrderRow(o, ctx);
    rows.push(raw);
    const old = have.get(raw.billz_id);
    if (!old || FIELDS.some((f) => !sameValue(old[f], raw[f]))) sahifadaYangi++;
    if (fetched % 100 === 0) {
  ...
  const { write, same, yangi, ozgargan } = onlyChanged(rows, have, (r) => r.billz_id, FIELDS);
  if (write.length) await upsertAll(db, "supplier_invoices", write, "company_id,billz_id");
  return {
    fetched, inserted: yangi, updated: ozgargan, unchanged: same,
    noStore: rows.filter((r) => !r.store_id).length,
    mode: full ? "full" : "incremental",
    cursor_at: rows[0]?.accepted_at ?? null,
  };
}

// ══════════════════════════════════════════════════════════════
// INVENTARIZATSIYA — Billz stocktaking → stocktakings (DAFTAR 24)
// ══════════════════════════════════════════════════════════════
// Yo'l (`/v2/stocktaking`) ochiq hujjatda yo'q edi — kabinetning o'zi
// kuzatilib topildi. Hujjat SARLAVHA darajasida (qatorlar API'da yo'q),
// ya'ni javob "qaysi tovar" emas, "qancha kamomad/ortiqcha va qancha
// summa" degan savolga.
//
// Hozircha 8 ta hujjat va HAMMASI `type=TRANSFER` — haqiqiy sanoq hali
// qilinmagan. Ko'zgu shunga qaramay quriladi (`supplier_invoices` bilan
// bir xil qaror, DAFTAR 20 E): sanoq boshlangan kuni raqam o'zi keladi.
// Ro'yxat kichik, shuning uchun DOIM to'liq tortiladi — inkremental
// murakkabligi bu yerda foyda bermaydi.
export async function syncStocktakings(db, ctx) {
  const FIELDS = ["external_id", "name", "store_id", "shop_name", "type", "status_id",
                  "qty", "new_products", "shortage", "surplus", "postponed",
                  "difference_sum", "process_percentage", "transfer_billz_id",
                  "created_by", "finished_by", "locked", "deleted", "created_at", "finished_at"];
  const existing = await readAll(db, "stocktakings",
    "id,billz_id," + FIELDS.join(","), (q) => q.not("billz_id", "is", null));
  const have = new Map(existing.map((r) => [r.billz_id, r]));

  const rows = [];
  let fetched = 0;
  for await (const s of stocktakingPages({ limit: 100 })) {
    fetched++;
    rows.push(stocktakingRow(s, ctx));
  }

  const { write, same, yangi, ozgargan } = onlyChanged(rows, have, (r) => r.billz_id, FIELDS);
  if (write.length) await upsertAll(db, "stocktakings", write, "company_id,billz_id");
  return {
    fetched, inserted: yangi, updated: ozgargan, unchanged: same,
    noStore: rows.filter((r) => !r.store_id).length,
    // Haqiqiy sanoq (transferga bog'liq bo'lmagan) nechta — ekranda
    // "inventarizatsiya qilinmagan" degan holat shu raqamdan ko'rinadi.
    sanoq: rows.filter((r) => String(r.type).toUpperCase() !== "TRANSFER").length,
    mode: "full",
    cursor_at: rows[0]?.created_at ?? null,
  };
}
...
export async function runSync(db, opts = {}) {
  ...
  const stages = [
    ...
    // Xarid hujjatlari (Billz supplier-order): birinchi marta to'liq
    ["supplierOrders", async () => syncSupplierOrders(db, ctx, {
        full: full || !(await lastCursor(db, ctx.companyId, "supplierOrders")),
      })],
    // Inventarizatsiya: ro'yxat kichik (8 hujjat), doim to'liq
    ["stocktakings", async () => syncStocktakings(db, ctx)],
  ];
