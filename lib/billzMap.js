...
// Bu qaror `lib/moslik.js` dagi "billz-price" tekshiruvi bilan
// mahkamlangan — narx birdan 11 880 barobar o'zgarsa darrov bilinadi.
...
export function matchStores(billzShops, nsposStores) {
  ...
  for (const sh of billzShops) {
    const id = byKey.get(shopKey(sh.name));
...
export function mergeCustomer(existing, incoming) {
  ...
  if (!out.first_purchase_at && existing.first_purchase_at) out.first_purchase_at = existing.first_purchase_at;
  if (!out.last_purchase_at && existing.last_purchase_at) out.last_purchase_at = existing.last_purchase_at;
...
export function debtPaymentRows(d, debtId) {
  ...
  for (const line of d.payments ?? []) {
    ...
    rows.push({
      ...
      amount: p.amount,
      paid_at: p.at,
      method: paymentKind(p.method) ?? (system ? "return" : "cash"),
