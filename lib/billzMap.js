 ...
 *
 * 2026-09-08: Billz interfeysi ("Сумма по цене поставки") partiyalar
 * bo'yicha so'nggi/haqiqiy kirim narxiga (`max_supply_price || min_supply_price`)
 * tayanadi. Ilgari faqat `min_supply_price` olingani sababli qoldiq qiymatida
 * $2 598 farq chiqardi.
 *
 * Shuningdek, metrli/mayda tovarlar (kabel, RJ45) narxi 1 dollardan
 * kichik ($0.0770) bo'lganda 2 xonaga yaxlitlansa 240 000 metrda
 * yuzlab dollar og'ish bo'ladi. Shuning uchun mayda/metrli tovarlarda
 * 4 xona aniqlik (`pul4`), oddiy tovarlarda 2 xona (`money`) saqlanadi.
 */
export function supplyPrice(p) {
  const rows = (p.product_supplier_stock ?? [])
    .map((x) => ({
      qty: num(x.measurement_value),
      cost: num(x.max_supply_price || x.min_supply_price),
    }))
    .filter((x) => x.cost > 0);
  if (!rows.length) {
    const sp = num(p.supply_price);
    return (sp > 0 && sp < 1) || (sp * 100) % 1 !== 0 ? pul4(sp) : money(sp);
  }

  const withQty = rows.filter((x) => x.qty > 0);
  const use = withQty.length ? withQty : rows.map((x) => ({ ...x, qty: 1 }));
  const qty = use.reduce((a, x) => a + x.qty, 0);
  const avg = use.reduce((a, x) => a + x.cost * x.qty, 0) / qty;
  return (avg > 0 && avg < 1) || (avg * 100) % 1 !== 0 ? pul4(avg) : money(avg);
}
...
export function mergeProduct(existing, incoming) {
  ...
  for (const k of keepIfZero) {
    if (!num(out[k]) && num(existing[k])) {
      const v = num(existing[k]);
      out[k] = (k === "cost_price" && (v < 1 || (v * 100) % 1 !== 0)) ? pul4(v) : money(v);
    }
  }
...
// ══════════════════════════════════════════════════════════════
// Sarlavha darajasida: `items` API'da doim null, hujjat yo'li yopiq.
// `type` Billz yorlig'i (`TRANSFER` | `INVENTORY` …) — u O'ZGARTIRILMAY
// saqlanadi va ma'no ekranda beriladi: kompaniyada hozircha hammasi
// `TRANSFER`, ya'ni haqiqiy sanoq hali qilinmagan.
export function stocktakingRow(s, ctx) {
  return {
    company_id: ctx.companyId,
    billz_id: s.id,
    external_id: Number.isFinite(+s.external_id) ? +s.external_id : null,
    name: clean(s.name) || null,
    store_id: ctx.storeByBillz.get(s.shop_id) ?? null,
    shop_name: clean(s.shop_name) || null,
    type: clean(s.type) || null,
    status_id: clean(s.status_id) || null,
    qty: num(s.total_measurement_value),
    new_products: num(s.new_products),
    shortage: num(s.shortage),
    surplus: num(s.surplus),
    postponed: num(s.postponed),
    difference_sum: num(s.difference_sum),
    process_percentage: num(s.process_percentage),
    // Billz bo'sh qiymatni "" bilan beradi — uuid ustuniga "" yozilsa
    // baza xato beradi, shuning uchun `uuid()` orqali null'ga aylanadi.
    transfer_billz_id: uuid(s.transfer_id) ?? null,
    created_by: clean(s.created_by?.name) || null,
    finished_by: clean(s.finished_by?.name) || null,
    locked: String(s.locked) === "true",
    deleted: String(s.deleted) === "true",
    created_at: billzTime(s.created_at) ?? new Date().toISOString(),
    finished_at: billzTime(s.finished_at),
    updated_at: new Date().toISOString(),
  };
}
