"use client";
import { allowedIn } from "./permsData";
// ══════════════════════════════════════════════════════════════
// ROLLAR VA RUXSATLAR
// ══════════════════════════════════════════════════════════════
...
export const PERMISSIONS = {
  ...
  // KPI, NPS, kurs — bazadagi siyosat AYNAN shu ro'yxat bilan
  // (`scripts/sql/huquq-2026-09.sql`, `npm run huquq` solishtiradi).
  // Retention xodimi NPS ni kassir rolida yozadi; KPI kunini har kim
  // O'ZIGA yozadi, boshqa xodimga esa faqat rahbar/menejer.
  "nps.edit": ["owner", "manager", "cashier"],
  "kpi.manage": ["owner", "manager"],
  "finance.rate": ["owner", "manager"],          // dollar kursi (`set_usd_rate`)
  ...
  "settings.edit": ["owner"],
  "staff.manage": ["owner"],
  // Do'kon menejeri kundalik ishni ustalar bilan yuritadi: yangi usta
  // kelganda unga login/parolni O'ZI ochadi, unutilgan parolni o'zi
  // tiklaydi. Faqat USTA hisobi — boshqa rol menejerga berilmaydi
  // (tekshiruv serverda: app/api/staff/route.js).
  "staff.installers": ["owner", "manager"],
};
...
// —— Tekshiruv ————————————————————————————————
// 2026-09-03: javob BAZADAN (`role_permissions`, rahbar Sozlamalarda
// belgilaydi) — RLS `has_perm()` ham aynan shu jadvaldan o'qiydi, ya'ni
// tugma bor-u baza rad etadigan holat bo'lmaydi. Jadval yuklanmagan
// bo'lsa PERMISSIONS (standart). Owner doim ruxsatli.
export function can(permission, user = currentUser) {
  const allowed = PERMISSIONS[permission];
  if (!allowed) return false;          // noma'lum huquq — ruxsat yo'q
  if (user?.role === "owner") return true;
  const bazadan = allowedIn(user?.role, permission);
  return bazadan ?? allowed.includes(user?.role);
}
...
// Sozlamalar → "Rollar va huquqlar" jadvalidagi yorliqlar (kalit → matn)
export const PERMISSION_LABELS = {
  "sale.create": "Sotuv qilish", "sale.return": "Qaytarish", "sale.viewAll": "Boshqa kassirlar cheklari", "sale.discount": "Chegirma berish",
  "product.view": "Tovarlarni ko'rish", "product.edit": "Tovar qo'shish/tahrirlash/arxivlash", "product.price": "Narx o'zgartirish",
  "warehouse.operate": "Ombor operatsiyalari", "warehouse.approve": "Inventarizatsiyani tasdiqlash",
  "customer.view": "Mijozlarni ko'rish", "customer.edit": "Mijoz qo'shish/tahrirlash", "customer.debt": "Qarz to'lovini qabul qilish", "customer.export": "Mijozlar bazasini yuklab olish",
  "kassa.view": "Kassalarni ko'rish", "kassa.operate": "O'z kassasiga kirim/chiqim", "kassa.transfer": "Rahbarga o'tkazma so'rash", "kassa.company": "Kompaniya balansidan chiqim", "kassa.approve": "O'tkazmani tasdiqlash",
  "finance.cash": "Kassa jurnali (qarz/balans/ta'minotchi to'lovi)", "finance.shift": "Smenalar", "finance.expenses": "Xarajat kiritish", "finance.payroll": "Ish haqi", "finance.plan": "Pul rejasi", "finance.pnl": "Foyda va zarar (P&L)", "finance.balance": "Balans", "finance.suppliers": "Ta'minotchilar va partiyalar", "finance.import": "Import",
  "service.view": "Xizmatlarni ko'rish", "service.edit": "Xizmat buyurtmasi tahriri", "service.own": "Faqat o'z buyurtmasi",
  "nps.edit": "NPS baho kiritish", "kpi.manage": "Boshqa xodimning KPI kunini yozish", "finance.rate": "Dollar kursini o'zgartirish",
  "report.view": "Hisobotlar", "report.margin": "Foyda ko'rinadigan hisobotlar",
  "settings.edit": "Sozlamalar", "staff.manage": "Xodimlar va vakolatlar", "staff.installers": "Ustaga login/parol ochish",
};
...
// Rol bo'yicha asosiy (kirish) sahifasi:
// Usta kirganda Ustalar reytingiga (/installers), omborchi omborga (/products),
// kassir sotuvga (/sales), rahbar va menejer esa Bosh sahifaga (/dashboard).
export function defaultRouteFor(user = currentUser) {
...
function allowedRaw(href, user) {
  ...
  // 1-bosqich — har xodimga alohida ruxsat (aniqdan umumiyga yuriladi).
  // Ota bo'lim yopilgan bo'lsa, bolalari ham yopiladi — agar bola uchun
  // alohida ruxsat berilmagan bo'lsa. 2026-09-03: bu ro'yxat faqat
  // TORAYTIRADI — rol (matritsa) ochmagan bo'limni ocholmaydi; ilgari
  // ochardi va sahifa ochilib, yozuv RLS'da rad bo'lardi.
  if (secs) {
    let p = href;
    while (p) {
      if (p in secs) return !!secs[p] && rolBoyicha(href, user);
  ...
  // 2-bosqich — rol (matritsa yoki standart)
  return rolBoyicha(href, user);
}

function rolBoyicha(href, user) {
  let path = href;
  while (path) {
