...
import { supabase, DEMO_MODE, insert } from "./db";
...
async function saveItems(op) {
  if (DEMO_MODE) return;
  await supabase.from("warehouse_items").delete().eq("operation_id", op.id);
  if (!op.items.length) return;
  await supabase.from("warehouse_items").insert(op.items.map((i) => ({
    operation_id: op.id,
