...
// —— Kimga to'lanadi ————————————————————————————
// Rahbar doim shu odamlar/firmalar bilan ishlaydi — har safar ismni
// qo'lda yozish xato manbai ("Ravshan EZVIZ" va "Ravshan ezviz" ikki
// xil odam bo'lib qolardi va tarix bo'linib ketardi. Ro'yxatdan
// tanlanadi; ro'yxatda yo'q bo'lsa "Boshqa" tanlanib, izoh MAJBURIY
// bo'ladi (kimga ketgani noma'lum summa qolmasin).
// Ro'yxatda "kimga" va "nima uchun" BIRGA: rahbar "Hiside China" deb
// o'ylaydi, "Tovar keltirish" deb emas. Tur (hisobot uchun) har nomning
// ortida turadi — alohida so'ralmaydi.
export const PAYOUT_PAYEES = [
  { name: "Ravshan EZVIZ", category: "goods" },
  { name: "Nuriddin KSS", category: "goods" },
  { name: "Ramziddin Darian", category: "goods" },
  { name: "Jo'rabek Qoqon", category: "goods" },
  { name: "Abdulaziz Perevod", category: "import" },
  { name: "Muhammad Vint Cosmos", category: "goods" },
  { name: "Isroil Qoqon Halowat", category: "goods" },
  { name: "Nurmuhammad", category: "goods" },
  { name: "Sobitxon", category: "goods" },
  // Xitoydagi yetkazib beruvchilar
  { name: "Hiside China", category: "goods" },
  { name: "Shunda China", category: "goods" },
  { name: "Wistino China", category: "goods" },
  { name: "Colorvu China", category: "goods" },
  { name: "Raina China", category: "goods" },
  { name: "Owire kabel China", category: "goods" },
  { name: "Bestecher China", category: "goods" },
  // Yo'lkira
  { name: "Import yo'lkira", category: "import" },
  { name: "Uzb yo'lkira", category: "import" },
];

// Nom ortidagi tur. Ro'yxatda yo'q nom uchun — tovar keltirish.
export const payeeCategory = (name) =>
  PAYOUT_PAYEES.find((p) => p.name === name)?.category ?? "goods";

export function payeeNames() {
  const used = items.map((p) => String(p.title || "").trim()).filter(Boolean);
  return [...new Set([...PAYOUT_PAYEES.map((p) => p.name), ...used])];
}
...
export function addPayout({
  ...
  put(p);
  // Bazadagi HAQIQIY id — "To'lash" darrov bosilganda o'sha kutiladi.
  // Vaqtinchalik id bilan yozilsa baza rad etadi va to'lov bazada
  // "rejada" bo'lib qolardi (keyin ikkinchi marta to'lanardi).
  p.saved = sync.created(p).then((r) => r?.data?.id ?? p.id);
  return p;
}
...
export function payPayout(id, { amount, amountSom = null, date, restDate = null, staffId = null } = {}) {
  ...
  // Ikkala yozuv ham bazaga tushib haqiqiy id olgach bog'lanadi:
  // to'lovning o'zi ham hali saqlanayotgan bo'lishi mumkin.
  Promise.all([op.saved, p.saved ?? Promise.resolve(done.id)]).then(([opId, payoutId]) => {
    if (opId && opId !== op.id) updatePayout(payoutId ?? done.id, { opId });
  });
