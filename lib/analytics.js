...
import { listProducts, allProducts, totalQty } from "./productsData";
import { skladId } from "./storesData";
import { categoryName } from "./categoriesData";
import { salesInRange, listSales, qatorTannarx } from "./salesData";
import { listCustomers } from "./customersData";
import { listDebts, ochiqQoldiq, qarzYoshiKun, yoshGuruhi, arBuckets as qarzYoshGuruhlari } from "./debtsData";
import { getReorderDays, getStockWindow } from "./companyData";
import { ymd } from "./dates";

const DAY = 86400000;

// ══════════════════════════════════════════════════════════════
// TOVAR KESIMI — kategoriya, brend, yetkazib beruvchi
// ══════════════════════════════════════════════════════════════
// Uchala qiymat BITTA joyda aniqlanadi: nomi ham, "yo'q" holati ham.
// Bo'sh qiymat TASHLANMAYDI — o'z nomi bilan variant bo'ladi
// ("Brendsiz"), aks holda brendsiz tovarlarni filtr bilan ajratib
// bo'lmasdi va ular hech bir tanlovga tushmay ko'rinmay qolardi.
export const tovarKesim = (p) => ({
  category: categoryName(p?.categoryId) ?? "Kategoriyasiz",
  brand: p?.brand || "Brendsiz",
  supplier: p?.supplier || "Yetkazib beruvchisiz",
});

const KESIM_KALIT = ["category", "brand", "supplier"];

// Tanlangan kesimga to'g'ri keladimi. `null`/bo'sh massiv = hammasi
// (MultiSelect qoidasi, CLAUDE.md 2026-09-02), ya'ni filtr qo'yilmagan
// chaqiruvchi (API, bot, "Buyurtma taklifi" kartasi) avvalgidek ishlaydi.
export function kesimMos(p, kesim) {
  if (!kesim) return true;
  const k = tovarKesim(p);
  return KESIM_KALIT.every((key) => {
    const v = kesim[key];
    if (!Array.isArray(v) || !v.length) return true;
    return v.some((x) => String(x) === k[key]);
  });
}

// Filtr variantlari — katalogda HAQIQATDA uchraydiganlar, alifbo
// tartibida. Xizmat (montaj) omborda turmaydi, shuning uchun u
// `stockCoverage` dagidek bu yerda ham hisobga olinmaydi.
export function kesimVariantlari() {
  const yigilgan = { category: new Set(), brand: new Set(), supplier: new Set() };
  for (const p of listProducts()) {
    if (p.isService) continue;
    const k = tovarKesim(p);
    for (const key of KESIM_KALIT) yigilgan[key].add(k[key]);
  }
  return Object.fromEntries(KESIM_KALIT.map((key) => [
    key, [...yigilgan[key]].sort().map((v) => ({ value: v, label: v })),
  ]));
}
...
export function stockCoverage({ windowDays = getStockWindow(), storeId = "all", storeIds = null, kesim = null, now = new Date() } = {}) {
  const from = new Date(now.getTime() - windowDays * DAY);
  const uzunKun = windowDays * 3;
  const uzunFrom = new Date(now.getTime() - uzunKun * DAY);
  // Do'kon tanlovi: `storeIds` (massiv, 2026-09-04 — Ombor qoplamasida
  // bir nechta do'kon birga) yoki eski `storeId` (bitta id / "all").
  // `null` = barcha do'kon. Eski chaqiruvchilar (reorderList, deadStock,
  // managementData) `storeId` bilan o'z holicha ishlaydi.
  const tanlangan = storeIds?.length ? storeIds : (storeId === "all" ? null : [storeId]);
  ...
  for (const s of salesInRange(uzunFrom, now)) {
    if (tanlangan && !tanlangan.includes(s.storeId)) continue;
  ...
  // Shu aylanishda OXIRGI MA'LUM TANNARX ham yig'iladi: katalogda 0
  // bo'lsa (Billz qoldig'i tugagan tovarga tannarx bermaydi, 02.09:
  // 43 tugagan tovardan 20 tasi) chek qatoridagi tannarx olinadi.
  const jami = new Map();
  const oxirgiKun = new Map();
  const oxirgiTannarx = new Map();      // productId → { kun, tannarx }
  for (const s of listSales()) {
    ...
    for (const i of s.items ?? []) {
      ...
      if (+i.costPrice > 0) {
        const t = oxirgiTannarx.get(i.productId);
        if (!t || kun > t.kun) oxirgiTannarx.set(i.productId, { kun, tannarx: +i.costPrice });
      }
    }
  }

  const bugun = ymd(now);
  // Filial tanlanganda SKLAD qoldig'i ham qatorda turadi: filialda 0,
  // Skladda 50 bo'lsa bu "buyurtma" emas, "ko'chirish" (2026-09-03).
  // Tanlovda Sklad bo'lsa yoki "Barcha" bo'lsa — kerak emas (null):
  // Sklad qoldig'i allaqachon yig'indi ichida.
  const sklad = skladId();
  const skladKerak = !!(tanlangan && sklad && !tanlangan.includes(sklad));
  return listProducts()
    ...
    // Kategoriya / brend / yetkazib beruvchi filtri ENG BOSHIDA:
    // qatorlar tashlab yuborilsa kartochka va grafiklar ham o'z-o'zidan
    // shu tanlovga moslashadi. Komponentda filtrlansa, yig'indi butun
    // katalog bo'yicha qolib ketardi va "10 ta tovar, 4 000 $" degan
    // bir-biriga to'g'ri kelmaydigan ekran chiqardi.
    .filter((p) => kesimMos(p, kesim))
    .map((p) => {
      const total = totalQty(p);
      const stock = tanlangan
        ? tanlangan.reduce((a, id) => a + (p.stock[id] ?? 0), 0)
        : total;
      const skladda = skladKerak ? (p.stock[sklad] ?? 0) : null;
      ...
      // QAYSI OYNA ISHLATILGANI qatorda YOZILADI (`oyna`): 02.09 da
      // "tugagan, lekin sotilyapti" 43 tovardan 25 tasi 30 kundan beri
      // umuman sotilmagan edi — ular 90 kunlik zaxira oyna orqali
      // "sotilyapti" bo'lib chiqqan va 178 $/kun "yo'qotish"ning 103 $ ini
      // bergan. Endi bu farq ko'rinadi va pul yig'indisiga kirmaydi.
      const oyna = q > 0 ? windowDays : uzunKun;
      ...
      // Tannarx: katalog → oxirgi chek qatori → noma'lum (null). NOL EMAS:
      // 0 "bepul" degani va foyda 100 % bo'lib chiqadi.
      const chek = oxirgiTannarx.get(p.id);
      const tannarx = +p.costPrice > 0 ? +p.costPrice : chek ? chek.tannarx : null;
      return {
        product: p, stock, avgDaily, daysLeft, oyna,
        // Kesim qatorning O'ZIDA — chaqiruvchi uni qaytadan
        // hisoblamasin (nomi va "yo'q" holati bir joyda, `tovarKesim`)
        ...tovarKesim(p),
        // Do'kon bo'yicha bo'linma — "Barcha" tanlanganda yig'indi
        // NIMADAN yig'ilgani ko'rinsin (Optim 3 · Namangan 0 · Sklad 12)
        stockByStore: p.stock,
        skladda,
        tannarx,
        tannarxManba: +p.costPrice > 0 ? "katalog" : chek ? "chek" : null,
...
export function productABC(from, to) {
  ...
  // Sotilgan tovar arxivda bo'lsa ham ABC'da qoladi — sotuv tarixi haqiqat
...
export function productProfit(from, to, { by = "product", storeId = "all" } = {}) {
  const catalog = allProducts();     // arxivlangan tovar ham — sotuv tarixi uchun
    ...
    for (const i of s.items ?? []) {
      ...
      if (!r) {
        r = { key: k, name: nomi(p, s), product: by === "product" ? p : null,
              qty: 0, revenue: 0, cogs: 0, tannarxsiz: 0, checks: new Set() };
        map.set(k, r);
      }
      ...
      r.qty += i.qty;
      r.revenue += +i.total || 0;
      // BITTA QOIDA — `salesData.qatorTannarx`: xizmat (montaj) 0 —
      // Billz'dagi 5 $ taxminiy, usta puli ish haqida; noma'lum → null,
      // u `tannarxsiz` ga tushadi va tannarxga 0 bo'lib kirmaydi.
      const { tannarx } = qatorTannarx(i, p);
      if (tannarx == null) r.tannarxsiz += +i.total || 0;
      else r.cogs += tannarx * i.qty;
      r.checks.add(s.id);
    }
  }
  ...
  const rows = [...map.values()].map((r) => {
    ...
    const tannarxsiz = +r.tannarxsiz.toFixed(2);
    return {
      ...r,
      revenue, cogs, profit, tannarxsiz,
      checks: r.checks.size,
      qty: +r.qty.toFixed(3),
      // Marja — foydaning TUSHUMGA nisbati. Tushum nol yoki manfiy
      // bo'lsa (faqat qaytarish bo'lgan tovar) marja ma'nosiz. Tannarxi
      // butunlay noma'lum tovarda ham (cogs 0, tannarxsiz > 0) — null,
      // 100 % emas.
      margin: revenue > 0 && !(cogs === 0 && tannarxsiz > 0) ? +((profit / revenue) * 100).toFixed(1) : null,
...
// Muddat (yetkazish + zaxira kuni) RAHBAR belgilaydi — `companies`
// jadvalida (`companyData.getReorderDays`), kodda qattiq emas.
export function reorderList({ windowDays = getStockWindow(), storeId = "all", kesim = null, leadDays = null, coverDays = null, now = new Date() } = {}) {
  const kun = getReorderDays();
  const lead = leadDays ?? kun.lead;
  const cover = coverDays ?? kun.cover;
  return stockCoverage({ windowDays, storeId, kesim, now })
    .filter((r) => r.avgDaily > 0 && (r.tugagan || (r.daysLeft != null && r.daysLeft <= lead)))
    .map((r) => {
      const kerak = Math.ceil(r.avgDaily * (lead + cover) - r.stock);
      // "Sokin": tugagan, lekin 30 kundan beri sotilmagan — tezlik
      // 90 kunlik zaxira oynadan. Ro'yxatdan OLINMAYDI (yashirilmaydi),
      // faqat alohida belgilanadi va pul yig'indisiga kirmaydi.
      const sokin = r.tugagan && r.oyna !== windowDays;
      const buyurtma = Math.max(kerak, 1);
      // Skladda bo'lsa — avval KO'CHIRISH, qolgani buyurtma (2026-09-03).
      // `buyurtma` formulasi o'zgarmaydi (API/bot shu raqamni oladi);
      // `kochirish` faqat filial tanlanganda > 0.
      const kochirish = r.skladda != null ? Math.min(Math.max(r.skladda, 0), buyurtma) : 0;
      return {
        ...r,
        sokin,
        buyurtma,
        kochirish,
        buyurtmaQoldiq: buyurtma - kochirish,
        ...
        // Faqat HAQIQATAN sotilayotgan (30 kunlik oyna) va tannarxi
        // MA'LUM tovar uchun son; aks holda `null` — 0 emas, chunki 0
        // "yo'qotish yo'q" degan yolg'on bo'lardi.
        kunlikYoqotish: !r.tugagan ? 0
          : sokin || r.tannarx == null ? null
          : +(r.avgDaily * ((r.product.salePrice || 0) - r.tannarx)).toFixed(2),
      };
    })
    .sort((a, b) => {
      if (a.tugagan !== b.tugagan) return a.tugagan ? -1 : 1;
      if (a.tugagan) {
        if (a.sokin !== b.sokin) return a.sokin ? 1 : -1;
        return (b.kunlikYoqotish ?? -1) - (a.kunlikYoqotish ?? -1);
      }
      return (a.daysLeft ?? 9999) - (b.daysLeft ?? 9999);
    });
}

// Kartochkalar — ekran, REST API va Telegram bot AYNAN shu yig'indini
// oladi ("bir tushuncha — bitta funksiya", CLAUDE.md 2026-08-14).
export function reorderSummary(rows) {
  const tugagan = rows.filter((r) => r.tugagan && !r.sokin);
  const kochirish = rows.filter((r) => r.kochirish > 0);
  return {
    buyurtma: rows.length,
    tugagan: tugagan.length,
    sokin: rows.filter((r) => r.sokin).length,
    tannarxsiz: tugagan.filter((r) => r.tannarx == null).length,
    yoqotishKun: +tugagan.reduce((a, r) => a + (r.kunlikYoqotish ?? 0), 0).toFixed(2),
    // Skladdan ko'chirish bilan yopiladiganlar (faqat filial tanlanganda)
    kochirishKerak: kochirish.length,
    kochirishDona: kochirish.reduce((a, r) => a + r.kochirish, 0),
    muddat: getReorderDays(),
  };
}
...
export function deadStock({ idleDays = 90, storeId = "all", kesim = null, now = new Date() } = {}) {
  ...
  // Oyna qotirib yozilmaydi — rahbar belgilagan sotuv oynasi
  // (`getStockWindow`), aks holda bir sahifada ikki xil "30 kun" bo'lardi
  const rows = stockCoverage({ storeId, kesim, now })
...
// Yosh guruhlari — Sozlamalar → Biznes qoidalari (`ar.buckets`, standart
// 30/60/90); id'lar o'zgarmaydi, chegaralar rahbarniki (2026-09-03).
// Yosh guruhlari ENDI `debtsData.arBuckets` da (2026-09-04, DAFTAR 20 B):
// Qarzdorlik sahifasi, AR aging, API va bot bitta ro'yxat. Bu yerda
// faqat eski importlar uchun qayta eksport.
export const arBuckets = qarzYoshGuruhlari;
export const AR_BUCKETS = arBuckets();   // eski importlar uchun (standart)
...
export function arAging({ now = new Date(), storeId = "all" } = {}) {
  ...
  const bucketJami = new Map(BUCKETS.map((b) => [b.id, { ...b, open: 0, count: 0 }]));
  // Jami — XOM yig'indi, oxirida bir marta yaxlitlanadi. Qarz endi 4
  // xona bilan yuritiladi (Billz kabi); mijoz qatorlari alohida
  // yaxlitlanib qo'shilsa 125 mijozda 3 tiyin farq chiqadi va
  // `moslik` → `ar-total` qizaradi (03.09 da shunday bo'ldi).
  let jamiOchiq = 0;
  ...
  for (const d of listDebts()) {
    ...
    // Yosh — `debtsData.qarzYoshiKun` (Qarzdorlik sahifasi bilan bitta)
    const yosh = qarzYoshiKun(d, now);
    const b = yoshGuruhi(yosh, BUCKETS);
  ...
  return {
    ...
    // Chegara — oxirgi yosh guruhi (sozlama), kodda qotgan 90 emas
    birinchi: rows.filter((r) => r.oldest >= BUCKETS.at(-1).min).sort((a, b) => b.open - a.open).slice(0, 10),
  };
}
