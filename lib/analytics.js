...
export function stockCoverage({ windowDays = sozlama("stock.windowDays", 30), storeId = "all", now = new Date() } = {}) {
  ...
  // Sklad tanlanganda yoki "Barcha" bo'lsa — kerak emas (null).
  const sklad = skladId();
  const skladKerak = storeId !== "all" && sklad && storeId !== sklad;
  return listProducts()
