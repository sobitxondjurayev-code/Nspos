...
export function syncTable(name, cfg) {
  ...
  registerModule(name, {
    table, readTable, select, order, realtime, oyna: cfg.oyna,
    fromRow,
    // Bazadan kelgan ro'yxat xotirani ALMASHTIRADI. Lekin shu paytda
    // saqlanayotgan yangi yozuv (`creating`) bazada hali yo'q —
    // almashtirishda u ro'yxatda QOLISHI kerak, aks holda xodim
    // kiritgan qator ekrandan yo'qolib, bir soniyadan keyin qaytib
    // kelardi. Og'ir jadval ikki to'lqinda kelgani uchun (`oyna`)
    // `restore` endi bir marta emas, ikki marta chaqiriladi.
    restore: (rows) => {
      const kutayotgan = get().filter((x) => creating.has(x.id));
      eslab(rows);
      qoy(arrange([...rows, ...kutayotgan]));
    },
