...
import { registerModule, insert, update as dbUpdate, remove as dbRemove, bumpData } from "./db";
...
export function syncTable(name, cfg) {
  ...
  registerModule(name, {
    ...
    fromRow,
    restore: (rows) => qoy(arrange(rows)),
    apply: ({ eventType, new: row, old }) => {
      if (eventType === "DELETE") return qoy(get().filter((x) => x.id !== old.id));
