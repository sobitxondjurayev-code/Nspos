...
// "Billz: 23:30" muhri uchun matn. Bugun bo'lsa faqat soat, aks holda
// kun ham — eskirgan ma'lumot bugungiga o'xshab turmasin.
export function billzVaqtMatni(iso) {
  if (!iso) return null;
  const d = new Date(iso);
  const p = (n) => String(n).padStart(2, "0");
  const soat = `${p(d.getHours())}:${p(d.getMinutes())}`;
  const bugun = new Date();
  const birKun = d.getFullYear() === bugun.getFullYear() && d.getMonth() === bugun.getMonth() && d.getDate() === bugun.getDate();
  return birKun ? soat : `${p(d.getDate())}.${p(d.getMonth() + 1)} ${soat}`;
}
