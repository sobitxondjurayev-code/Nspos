...
export const remainingOf = (d) => +(d.amount - tolangan(d)).toFixed(2);
...
// Muddati o'tganmi — Billz holati bo'lsa o'sha (`overdue`), qo'lda
// ochilgan qarzda to'lov muddatidan. Kartochka ham, matritsa ham shu.
export const muddatiOtganmi = (d, bugun = ymd(new Date())) =>
  d.status ? d.status === "overdue" : !!(d.dueDate && d.dueDate < bugun);
...
export function jamiQarz(now = new Date()) {
  ...
  for (const d of listDebts()) {
    ...
    if (!q) continue;
    out.jami += q; out.soni++;
    const b = muddatiOtganmi(d, bugun) ? out.muddatiOtgan : out.muddatiKelmagan;
    b.summa += q; b.soni++;
    if (tolangan(d) > 0) { out.qismanTolangan.summa += q; out.qismanTolangan.soni++; }
  }
  out.jami = +out.jami.toFixed(2);
  for (const k of ["muddatiOtgan", "muddatiKelmagan", "qismanTolangan"]) out[k].summa = +out[k].summa.toFixed(2);
  return out;
}
