...
// Qarzlar BAZADAN o'qiladi.
//
// 2026-08-19 gacha bu yerda Excel yuklamasi ustun turardi, chunki
// bazadagi eski qarzlarda qaytish sanalari MODELLASHTIRILGAN edi.
// Endi Billz API haqiqiy raqamni beradi: 10 837 qarz va 15 987 to'lov,
// har birida qachon va qanday usulda to'langani bor.
//
// `source = 'excel'` qatorlar SANALMAYDI. Ular Excel importidan qolgan
// eskirgan surat (07.06–22.07, hammasi "ochiq" deb turibdi, jami
// 103 380 $) — Billz'da esa ularning ko'pi allaqachon to'langan va
// haqiqiy ochiq qarz 52 502 $. Ikkalasi birga sanalsa ochiq qarz
// ikki barobar ko'p ko'rinadi. O'CHIRILMAYDI — tekshiruv uchun
// bazada qolaveradi (foydalanuvchi qarori, 2026-08-19).
export const listDebts = () => {
  const billz = debts.filter((d) => d.source === "billz");
  if (billz.length) return billz;
  if (debts.length) return [...debts];
  const up = uploadedDebts();
  return up.ready ? [...up.rows] : [];
};
...
export const debtsFromBillz = () =>
  debts.some((d) => d.source === "billz") || uploadedDebts().ready;
