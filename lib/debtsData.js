...
// Davrda QARZGA QAYTARILGAN tovar summasi (`kind = 'return'`,
// "Системная оплата") — pul kelmadi, lekin qarz kamaydi. Foyda → pul
// ko'prigida (`pnlData.foydaPulKoprigi`) nasiya o'sishini hisoblashda
// kerak: qarz o'sishi = berilgan − to'langan − qaytarilgan.
export function qarzgaQaytarish(from, to) {
  const [a, b] = oraliq(from, to);
  let total = 0, count = 0;
  for (const d of listDebts()) {
    for (const p of d.payments) {
      if (p.kind !== "return") continue;
      const t = new Date(p.at).getTime();
      if (t >= a && t <= b) { total += p.amount; count++; }
    }
  }
  return { total: +total.toFixed(2), count };
}

// QARZ HOLATI BERILGAN SANADA (oy muhri uchun). `ochiqQoldiq` HOZIRGI
// holat (Billz `paid_amount`); o'tgan oy oxiridagi holat esa to'lov
// YOZUVLARIDAN quriladi: sanagacha berilgan qarz − sanagacha tushgan
// to'lov va qaytarish. `moslik` → `debt-paid` to'lov yozuvlari Billz
// `paid_amount` bilan tengligini kafolatlaydi.
export function qarzHolati(asOf = new Date()) {
  const t0 = new Date(asOf).getTime();
  let jami = 0, soni = 0;
  for (const d of listDebts()) {
    if (new Date(d.createdAt).getTime() > t0) continue;
    let tolangan = 0;
    for (const p of d.payments) if (new Date(p.at).getTime() <= t0) tolangan += p.amount;
    const q = +(d.amount - tolangan).toFixed(4);
    if (q > 0.009) { jami += q; soni++; }
  }
  return { jami: +jami.toFixed(2), soni };
}
