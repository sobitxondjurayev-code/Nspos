...
// Token qachon tugaydi (ms). Sessiya yo'q bo'lsa 0.
//
// NEGA KERAK (2026-08-24): token 12 soat yashaydi, `lib/db.js` esa
// uni modul yuklanganda BIR MARTA o'qiydi. Ochiq turgan oynada muddat
// tugasa, ilova buni sezmasdan so'rov yuboraverardi — PostgREST esa
// har biriga 401 berardi. Ekranda na xato, na bo'sh sahifa: eski
// raqamlar turaverardi va yangisi kelmasdi. Nginx jurnalida bu
// kuniga o'nlab `401 GET /rest/v1/…` bo'lib ko'rinardi.
export function muddatMs() {
  const t = tokenOl();
  if (!t) return 0;
  try {
    const p = JSON.parse(atob(t.split(".")[1].replace(/-/g, "+").replace(/_/g, "/")));
...
export async function kirish(email, parol) {
  ...
  xabar();
  return { error: null, role: j.role ?? null };
}
