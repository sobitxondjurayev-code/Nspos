...
function fillDays(a, b, row) {
  ...
  for (const d of listAllDays(a, b)) {
    ...
    // Usta olgan pul — servis pulidan beriladi, shuning uchun
    // "Servis xarajatlar" ustunida (balansdan ham ayriladi).
  ...
  for (const e of expensesInRange(new Date(a + "T00:00:00"), new Date(b + "T23:59:59"))) {
    const r = row(e.date);
    // SERVIS qopidan chiqqan hammasi "Servis xarajatlar"da — material
    // ham, USTA OYLIGI ham (rahbar qoidasi 2026-08-13). Shunda servis
    // kirimidan shu ustunni ayirsa servis hamyonidagi pul chiqadi.
    // Boshqa qopdan berilgan oylik (menejer, kassir) esa avvalgidek
    // "Oylik maoshlar" ustunida qoladi.
    if (e.method === "service") r.svcExp += e.amount;
    else if (e.category === "salary") r.salary += e.amount;
  ...
  for (const o of ops) {
    ...
    const r = row(o.date);
    if (o.wallet === "service") r.svcExp += o.amount;
    else if (o.category === "salary") r.salary += o.amount;
...
export function flowSources(from, to, key, opts = {}) {
  ...
  for (const d of listAllDays(a, b)) {
    ...
    // Usta olgan pul — servis xarajati
  ...
  for (const e of expensesInRange(new Date(a + "T00:00:00"), new Date(b + "T23:59:59"))) {
    ...
    const col = e.method === "service" ? "svcExp"
  ...
  for (const o of ops) {
    ...
    const col = o.wallet === "service" ? "svcExp"
      : o.category === "salary" ? "salary"
