...
function fillDays(a, b, row) {
  ...
  for (const d of listAllDays(a, b)) {
    ...
    // Usta olgan pul — bu ham oylik, "Oylik maoshlar" ustunida turadi.
    // Puli servis hamyonidan chiqadi: BALANSDAN ayriladi
    // (kassaBalances), lekin xarajat ustuniga qo'shilmaydi.
  ...
  for (const e of expensesInRange(new Date(a + "T00:00:00"), new Date(b + "T23:59:59"))) {
    const r = row(e.date);
    // Oylik o'z ustunida turadi (rahbar qoidasi) — qop ustuniga
    // qo'shilmaydi, ya'ni ustunlarni qo'shsa bo'ladi. Shu sababli
    // "Servis xarajatlar" servis hamyonidagi puldan kam bo'lishi
    // mumkin: oylik ham o'sha hamyondan chiqadi, lekin boshqa ustunda.
...
export function flowSources(from, to, key, opts = {}) {
  ...
  for (const e of expensesInRange(new Date(a + "T00:00:00"), new Date(b + "T23:59:59"))) {
    ...
    const col = e.category === "salary" ? "salary" : potCol(e.method, e.kassa, b2b);
    if (col !== key) continue;
  ...
  for (const o of ops) {
    ...
    const col = o.category === "salary" ? "salary"
