...
function dayMapOf(kassaId, a, b) {
  ...
  if (!isCompany) {
    ...
    for (const d of listAllDays(a, b)) {
      ...
      for (const [w, v, lbl] of [
        ...
        if (!v) continue;
        r.in[w] += v;
        r.src.push({ date: d.date, title: who, note: lbl, amount: v });
      }
    }
  }
  ...
  for (const e of expensesInRange(new Date(a + "T00:00:00"), new Date(b + "T23:59:59"))) {
    ...
    const r = row(e.date);
    r.out[walletOf(e.method)] += e.amount;
  ...
  for (const o of listOperations()) {
    ...
    const r = row(day);
    r.out[walletOf(o.method)] += o.amount;
  ...
  for (const o of ops) {
    ...
    if (mine && o.kind === "in") {
      ...
      r.src.push({ date: o.date, title: categoryLabel(o.category), note: o.note || "",
        amount: o.amount, amountSom: o.amountSom ?? null });
      ...
      if (mine) {
        ...
        r.src.push({ date: o.date, title: KASSAS[o.kassa]?.label ?? o.kassa,
          note: o.category === CLOSE ? "Kassa yopildi" : "Rahbarga o'tkazma", amount: o.amount });
      }
    }
  }
...
export function dayRemainder(kassaId, date) {
  ...
  for (const w of walletsOf(kassaId)) {
    const v = m ? m.in[w] - m.out[w] - m.sent[w] - m.wait[w] : 0;
...
// Bir kun ortidagi yozuvlar — jadvaldagi "Kirim"/"Chiqim" ustiga
// bosilganda ochiladi. Jadval raqami bilan bitta manbadan.
export function kassaDaySources(kassaId, date, kind = "all") {
  const d = iso(date);
  const m = dayMapOf(kassaId, d, d).get(d);
  if (!m) return [];
  return m.src.filter((s) => kind === "all" || (kind === "out" ? s.out : !s.out));
}
