...
function fillDays(a, b, row) {
  ...
  for (const d of listAllDays(a, b)) {
    ...
    // Usta olgan pul — puli SERVIS hamyonidan chiqadi, shuning uchun
    // faqat "Servis xarajatlar" ustunida turadi. "Oylik maoshlar"da
    // ko'rsatilmaydi (rahbar qoidasi, 2026-08-13): aks holda bir pul
    // ikki ustunda ko'rinib, ustunlarni qo'shib bo'lmasdi.
...
export function flowSources(from, to, key, opts = {}) {
  ...
  for (const d of listAllDays(a, b)) {
    ...
    // Usta olgan pul — servis pulidan chiqadi (oylik ustunida emas)
    if (key === "svcExp" && ustalar.has(d.staffId) && d.olgan) {
