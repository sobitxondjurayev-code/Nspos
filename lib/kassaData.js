...
import { servisKirim } from "./serviceIncome";
import { billzKassaFlow, billzKassaDays, kunlikKirim, kunlikKirimStore, kirimYigindisi } from "./kassaIncome";
...
// `payme` hamyoni — BARCHA naqdsiz pul: Payme, Click, Uzum, Humo/Uzcard
// terminal. Billz to'lov turini erkin nomlaydi (`billzMap.paymentKind` →
// `card`/`payme`), lekin kassa uchun ikkalasi bir hamyon — bank hisobi.
// 2026-09-04 gacha `card` HECH QAYSI hamyonga tushmasdi (DAFTAR 20).
export const WALLETS = { cash: "Naqd", payme: "Payme / karta", service: "Servis" };
...
export const KASSA_CATEGORIES = {
  ...
  out: {
    // Nomi ATAYLAB aniqlashtirildi (2026-09-05): "Import xarajati" degan
  // yorliq menejerni yo'lkirani shu yerga yozishga chorlardi, holbuki
  // `goods`/`import` chiqimlari P&L ga UMUMAN kirmaydi (ombor aktivi).
  // Yo'lkiraning joyi endi Xarajatlar → "Import yo'lkira" (u tannarxga
  // qo'shiladi). Kalit `import` O'ZGARMAYDI — eski yozuvlar shunda.
  import: "Import to'lovi (tovar uchun)",
...
// —— O'tkazmalar Xarajatlar ro'yxatida ————————————————
// Rahbar qoidasi (2026-08-15): rahbarga qilingan pul o'tkazmasi ham
// Xarajatlar ro'yxatida ko'rinib turishi kerak. Lekin u XARAJAT EMAS —
// pul kompaniyada qoladi, shuning uchun expensesInRange'ga QO'SHILMAYDI
// (aks holda P&L, kassa balansi va moslik tekshiruvi uni ikki marta
// sanardi). Bu funksiya faqat ro'yxat sahifasi uchun xarajat
// ko'rinishidagi qatorlar qaytaradi; sahifa ularni Jami'ga qo'shmay,
// alohida ko'rsatadi. Kunlik yopilish (category='close') olinmaydi —
// u kunning butun tushumi, ro'yxatni bosib yuborardi.
export function ownerTransferRows(from, to) {
  return listOps(from, to)
    .filter((o) => o.kind === "transfer" && o.category !== CLOSE && o.status !== "rejected")
    .map((o) => ({
      id: "transfer:" + o.id,
      date: o.date,
      category: "owner_transfer",
      storeId: o.kassa === COMPANY ? "all" : o.kassa,
      amount: o.amount,
      amountSom: o.amountSom ?? null,
      rateUsed: o.rateUsed ?? null,
      method: o.wallet,
      kassa: o.kassa,
      staffId: null,
      paidTo: "Rahbar",
      note: o.note || "",
      createdBy: o.staffId ?? null,
      source: "transfer",
      status: o.status,
    }));
}
...
export function kassaBalances(asOf = new Date()) {
  ...
  //
  //    `servisKirim` — yagona kirish nuqtasi: jonli chek qatorlari
  //    bo'lsa o'shandan (kunma-kun aniq), bo'lmasa Excel yuklamasidan.
  if (kpiService === 0) {
    out[COMPANY].service += servisKirim(from, to).revenue;
  }
...
export function moneyFlow(from, to) {
  ...
  const service = kpiService || servisKirim(a, b).revenue;
  const opsIn = +ops
...
function fillDays(a, b, row) {
  ...
  // Kirim — Billz cheklaridan (2026-08-26 dan; ilgari menejerning
  // kunlik jadvalidan qo'lda kelardi va u 20-avgustdan beri bo'sh
  // turgan). KPI jadvali ham AYNAN shu funksiyadan o'qiydi
  // (`kpiData.billzKunlari`) — ikki ekranda ikki xil raqam bo'lmasin.
  for (const [store, kunlar] of Object.entries(kunlikKirim(a, b))) {
    ...
    for (const [date, w] of Object.entries(kunlar)) {
      ...
      r.cash += w.cash;
      r.payme += w.payme;
      if (b2b.has(store)) r.b2b += w.cash + w.payme; else r.b2c += w.cash + w.payme;
      // Servis kirimi — o'rnatish xizmatidan tushgan pul. Do'kon
      // savdosi emas (montaj naqddan ayrib olinadi), shuning uchun
      // b2c ga qo'shilmaydi — o'z ustuni bor va servis xarajatlari
      // shundan chiqadi.
      r.service += w.service;
    }
  }
  ...
  // NSPOS ichidagi eski to'lovlar jurnali (cash_operations). Hozir u
  // bo'sh, lekin kassa balansida ham, kassaning kunlik jadvalida ham
  // sanaladi — bu yerda ham sanalishi kerak, aks holda yozuv paydo
  // bo'lgan kuni kartochka bilan jadval bir-biriga qarshi chiqadi.
  for (const o of listOperations()) {
    if (o.type !== "chiqim") continue;
    const day = String(o.at).slice(0, 10);
    ...
    const r = row(day);
    const w = WALLET_IDS.includes(o.method) ? o.method : "cash";
    if (o.category === "salary" && w === "service") r.salary += o.amount;
    else if (w === "service") r.svcExp += o.amount;
    else r[potCol(w, o.storeId, b2b)] += o.amount;
  }
  ...
  for (const o of ops) {
    if (o.date < a || o.date > b) continue;
    // Kassaga qo'lda kiritilgan KIRIM ham ustunda ko'rinishi kerak:
    // u kartochkadagi "Kirgan pul" ichida bor (moneyFlow), demak
    // jadvalda ko'rinmasa ikki raqam bir-biriga qarshi bo'lardi.
    if (o.kind === "in") {
      const r = row(o.date);
      if (o.wallet === "service") r.service += o.amount;
      else {
        r[o.wallet] += o.amount;                 // Naqd / Payme ustuni
        if (b2b.has(o.kassa)) r.b2b += o.amount; else r.b2c += o.amount;
      }
      continue;
    }
...
export function flowSources(from, to, key, opts = {}) {
  ...
  // 1) Kirim — Billz cheklari va qarz to'lovlari (`fillDays` bilan
  //    AYNAN bir manba, aks holda katakdagi raqam ochilgan ro'yxat
  //    yig'indisiga teng bo'lmay qolardi — `moslik.js` → `kassasrc`).
  //
  //    Sarlavhada odam emas, DO'KON turadi: Billz chekni do'konga
  //    bog'laydi, menejerga emas. Ilgari bu yerda menejer ismi turardi,
  //    chunki raqamni o'sha kiritardi.
  for (const [store, kunlar] of Object.entries(kunlikKirim(a, b))) {
    if (!kassaIds().includes(store)) continue;
    if (onlyKassa && store !== onlyKassa) continue;
    const nom = KASSAS[store]?.label ?? store;
    const isB2b = b2b.has(store);
    for (const [date, w] of Object.entries(kunlar)) {
      if (key === "b2b" && isB2b) add({ date, title: nom, note: "Billz cheklari · naqd + Payme", amount: w.cash + w.payme });
      if (key === "b2c" && !isB2b) add({ date, title: nom, note: "Billz cheklari · naqd + Payme", amount: w.cash + w.payme });
      if (key === "cash") add({ date, title: nom, note: "Billz cheklari · naqd", amount: w.cash });
      if (key === "payme") add({ date, title: nom, note: "Billz cheklari · Payme", amount: w.payme });
      if (key === "service") add({ date, title: nom, note: "Billz cheklari · servis (montaj)", amount: w.service });
    }
  }
  ...
  for (const e of expensesInRange(new Date(a + "T00:00:00"), new Date(b + "T23:59:59"))) {
    ...
    add({ date: e.date, out: true,
      ...
      link: e.source === "installer" ? "/kpi"
        : e.source === "recurring"
          ? `/finance/expenses?tab=doimiy&edit=${e.recurringId}`
          : `/finance/expenses?edit=${e.id}` });
  }

  // 3) Eski to'lovlar jurnali (cash_operations) — fillDays bilan bir xil
  for (const o of listOperations()) {
    if (o.type !== "chiqim") continue;
    const day = String(o.at).slice(0, 10);
    if (day < a || day > b) continue;
    if (onlyKassa && kassaOf(o.storeId) !== onlyKassa) continue;
    const w = WALLET_IDS.includes(o.method) ? o.method : "cash";
    const col = o.category === "salary" && w === "service" ? "salary"
      : w === "service" ? "svcExp"
      : potCol(w, o.storeId, b2b);
    if (col !== key) continue;
    add({ date: day, out: true, title: "To'lov", note: o.note || "", amount: o.amount });
  }

  // 4) Kassa yozuvlari: qo'lda kiritilgan kirim, ish haqi to'lovi,
  //    rahbar olgan shaxsiy pul va Pul rejasidan to'langanlar (tovar
  //    keltirish, import…). Ustunga ajratish fillDays bilan BIR XIL —
  //    aks holda ochilgan ro'yxatning yig'indisi katakdagi raqamga
  //    to'g'ri kelmasdi.
  for (const o of ops) {
    if (o.date < a || o.date > b) continue;
    if (onlyKassa && o.kassa !== onlyKassa) continue;
    // Qo'lda kiritilgan kirim — jadvalda ustunga tushdi, demak
    // ochilgan ro'yxatda ham bo'lishi shart
    if (o.kind === "in") {
      const cols = o.wallet === "service"
        ? ["service"]
        : [o.wallet, b2b.has(o.kassa) ? "b2b" : "b2c"];
      if (!cols.includes(key)) continue;
      add({ date: o.date, title: categoryLabel(o.category),
        note: [KASSAS[o.kassa]?.label, o.note].filter(Boolean).join(" · "),
        amount: o.amount, amountSom: o.amountSom ?? null, link: "/finance/kassa" });
      continue;
    }
    if (o.kind !== "out") continue;
...
// —— Do'kon kassasining kirimi ————————————————————
// Naqd · Payme · Servis — Billz cheklari va qarz to'lovlaridan
// (`kassaIncome.kirimYigindisi`). KPI kunlik jadvali ham AYNAN shu
// manbadan o'qiydi, ya'ni ikki ekranda ikki xil raqam chiqmaydi.
//
// NEGA QO'LDAN OLINMAY QOLDI (2026-08-26). Ilgari menejer har kuni
// KPI jadvaliga o'zi yozardi va o'sha raqamdan inkassatsiya bonusi
// hisoblanardi. Bazadan o'lchandi: menejer amalda Billz raqamini
// ko'chirib yozar ekan (servis 19 kundan 18 tasida tiyinigacha teng),
// ya'ni qo'l mehnati ma'lumot qo'shmasdi — faqat kechikish qo'shardi.
// 20-avgustdan 26-igacha esa hech kim yozmadi va kassa kirimi yetti
// kun bo'sh turdi (Namangan 12 218.51 $ · Optim 15 470.96 $).
//
// Eski qo'lda yozilgan qatorlar bazada JOYIDA qoladi va kamomad
// jadvali (`kassaControlDays`) hamon o'shani Billz bilan solishtiradi.
export function kpiIncome(from, to) {
  const out = {};
  for (const [store, k] of Object.entries(kirimYigindisi(from, to))) {
    // Do'koni kassa bo'lmagan yozuv (masalan Sklad) kassaga tushmaydi
    if (!kassaIds().includes(store)) continue;
    out[store] = k;
  }
  return out;
...
//
// ⚠ 2026-08-26 DAN: KPI jadvalidagi Naqd/Payme/Servis Billz'dan
// avtomat to'ladi, ya'ni `kpiIncome()` endi BILLZ'NING O'ZI. Uni bu
// yerda ishlatib bo'lmaydi — farq har doim 0 chiqib, jadval "kamomad
// yo'q" deb ishontirib turardi (DAFTAR 10.4: yolg'on jurnal haqiqiy
// nosozlikni yashiradi). Shuning uchun bu yerda XOM, QO'LDA yozilgan
// qatorlar o'qiladi: 1–19 avgust tarixi hamon solishtiriladi,
// qo'lda yozilmagan kun esa "solishtirilmagan" bo'lib turadi.
function qolYozgani(from, to) {
  ...
  for (const d of listAllDays(from, to)) {
    ...
    if (!store) continue;
    if (bosh(d.cash) && bosh(d.payme) && bosh(d.service)) continue;
    const k = (out[store] ??= { cash: 0, payme: 0, service: 0, kunlar: new Set() });
...
export function kassaControl() {
  ...
  const rows = kassaIds()
    ...
    .map((k) => {
      ...
      const short = +solish.filter((d) => d.diff < -0.01)
        .reduce((a, d) => a + d.diff, 0).toFixed(2);
      const over = +solish.filter((d) => d.diff > 0.01)
        .reduce((a, d) => a + d.diff, 0).toFixed(2);
      // Umumiy "Billz" ustuni ham FAQAT solishtirilgan kunlar bo'yicha
      // olinadi. Aks holda menejer yozmagan kunlarning puli farqqa
      // qo'shilib, hech qachon bo'lmagan kamomad chiqardi.
      const billzSolish = +solish.reduce((a, d) => a + d.billz, 0).toFixed(2);
      return {
        kassa: k,
        label: KASSAS[k]?.label ?? k,
        billz, said,
        // Manfiy — menejer Billz ko'rsatgandan KAM topshirgan.
        // Solishtirilgan kun bo'lmasa — `null` ("—" bo'lib ko'rinadi),
        // NOL emas: nol "hammasi joyida" degan yolg'on xabar berardi.
        diff: solish.length ? +(said - billzSolish).toFixed(2) : null,
        short, over,
        shortDays: solish.filter((d) => d.diff < -0.01).length,
        overDays: solish.filter((d) => d.diff > 0.01).length,
        pct: solish.length && billzSolish > 0
          ? +(((said - billzSolish) / billzSolish) * 100).toFixed(2) : null,
        hasKpi: said > 0,
        // Nechta kun solishtirildi / umuman nechta kun bor
        solishtirilgan: solish.length,
        kunlar: days.length,
      };
    });
...
export function kassaControlDays(kassaId) {
  ...
  // Menejer kunlik jadvalga QO'LDA yozgani — shu kassaga tegishli
  // xodimlarniki. Bu ATAYLAB xom `listAllDays()`: KPI jadvalidagi
  // ustunlar 2026-08-26 dan Billz'dan avtomat to'ladi, ya'ni
  // `listDays()` orqali o'qilsa ikkala tomon bir manba bo'lib qolardi
  // va farq har doim 0 chiqardi.
  const said = {};
  for (const d of listAllDays(from, to)) {
    if (staffStore.get(d.staffId) !== kassaId) continue;
    // Uchala katak ham bo'sh — bu kun umuman to'ldirilmagan.
    // Uni nol deb hisoblash mumkin emas: o'shanda Billz'ning butun
    // kunlik puli "kamomad" bo'lib chiqardi.
    if (bosh(d.cash) && bosh(d.payme) && bosh(d.service)) continue;
  ...
  return days.map((date) => {
    ...
    const k = said[date] ?? null;
    const billz = +(b.cash + b.payme).toFixed(2);
    const kpi = k ? +(k.cash + k.payme + k.service).toFixed(2) : null;
    return {
      date, billz, kpi,
      billzCash: b.cash, billzPayme: b.payme,
      kpiCash: k ? +k.cash.toFixed(2) : null,
      kpiPayme: k ? +k.payme.toFixed(2) : null,
      kpiService: k ? +k.service.toFixed(2) : null,
      // `null` — o'sha kun qo'lda to'ldirilmagan, ya'ni solishtiradigan
      // ikkinchi manba yo'q. Nol qaytarilsa "farq yo'q" degan yolg'on
      // xabar bo'lardi.
      diff: k ? +(kpi - billz).toFixed(2) : null,
    };
  });
}
...
function dayMapOf(kassaId, a, b) {
  ...
  // 1) Kirim — Billz cheklari va qarz to'lovlari (do'kon kassalari).
  //    `fillDays` va `flowSources` bilan AYNAN bir manba.
  if (!isCompany) {
    const nom = KASSAS[kassaId]?.label ?? kassaId;
    for (const [date, w] of Object.entries(kunlikKirimStore(kassaId, a, b))) {
      const r = row(date);
      for (const [wallet, v, lbl] of [
        ["cash", w.cash, "Billz cheklari · naqd"],
        ["payme", w.payme, "Billz cheklari · Payme"],
        ["service", w.service, "Billz cheklari · servis (montaj)"],
      ]) {
        if (!v) continue;
        r.in[wallet] += v;
        addSrc(r, { date, title: nom, note: lbl, amount: v, bucket: "in", wallet });
      }
    }
  }
...
// —— Kunlik balans dinamikasi (grafik uchun) ————————————
// Har kun uchun `kassaBalances` NING O'ZI chaqiriladi — bu yerda
// qayta hisob YOZILMAYDI. Sekinroq (30 kun = 30 chaqiruv), lekin
// grafikdagi chiziq kassa sahifasidagi raqam bilan HAR DOIM bir xil
// bo'ladi. Qayta yozilsa ular asta ajralib ketardi va buni hech kim
// sezmasdi (DAFTAR 2026-08-14 dagi hodisa).
export function kassaBalanceSeries(from, to) {
  const a = new Date(from); a.setHours(0, 0, 0, 0);
  const b = new Date(to); b.setHours(23, 59, 59, 999);
  const kunlar = [];
  for (let d = new Date(a); d <= b; d.setDate(d.getDate() + 1)) {
    const kun = new Date(d); kun.setHours(23, 59, 59, 999);
    const bal = kassaBalances(kun);
    const jami = +Object.values(bal).reduce((s, v) => s + v.total, 0).toFixed(2);
    // Kamomad: qaysidir hamyon manfiyga tushgan kun. Bu "pul kam"
    // degani emas — "hisob-kitobda teshik bor" degani, ya'ni boshqa
    // savol va boshqa rang.
    const kamomad = +Object.values(bal)
      .flatMap((v) => [v.cash, v.payme, v.service])
      .filter((x) => x < 0)
      .reduce((s, x) => s + x, 0).toFixed(2);
    kunlar.push({
      sana: `${String(kun.getDate()).padStart(2, "0")}.${String(kun.getMonth() + 1).padStart(2, "0")}`,
      iso: `${kun.getFullYear()}-${String(kun.getMonth() + 1).padStart(2, "0")}-${String(kun.getDate()).padStart(2, "0")}`,
      qoldiq: jami,
      kamomad,
    });
  }
