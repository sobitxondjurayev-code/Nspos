...
function fillDays(a, b, row) {
  ...
  for (const d of listAllDays(a, b)) {
    ...
    // Usta olgan pul — bu ham oylik. Puli SERVIS hamyonidan chiqadi,
    // shuning uchun "Servis xarajatlar" ustuniga ham tushadi (qop
    // bo'yicha), "Oylik maoshlar" esa "shundan qanchasi oylik" bo'lib
    // qoladi — Naqd/Payme kabi.
    if (ustalar.has(d.staffId) && d.olgan) {
      const usd = somToUsd(d.olgan);
      if (usd) { r.salary += usd; r.svcExp += usd; }
    }
  }
  ...
  for (const e of expensesInRange(new Date(a + "T00:00:00"), new Date(b + "T23:59:59"))) {
    const r = row(e.date);
    r[potCol(e.method, e.kassa, b2b)] += e.amount;
  ...
  for (const o of ops) {
    ...
    const r = row(o.date);
    r[potCol(o.wallet, o.kassa, b2b)] += o.amount;
    if (o.category === "salary") r.salary += o.amount;
...
export function flowSources(from, to, key, opts = {}) {
  ...
  for (const d of listAllDays(a, b)) {
    ...
    // Usta olgan pul ham oylik hisoblanadi va servis pulidan chiqadi
    if ((key === "salary" || key === "svcExp") && ustalar.has(d.staffId) && d.olgan) {
  ...
  for (const e of expensesInRange(new Date(a + "T00:00:00"), new Date(b + "T23:59:59"))) {
    ...
    // Ustun ikki xil bo'lishi mumkin: qop ustuni (pul qayerdan chiqdi)
    // va "Oylik maoshlar" — ular ustma-ust tushadi, chunki oylik ham
    // biror qopdan chiqadi.
    if (key !== potCol(e.method, e.kassa, b2b)
        && !(key === "salary" && e.category === "salary")) continue;
  ...
  for (const o of ops) {
    ...
    if (key !== potCol(o.wallet, o.kassa, b2b)
        && !(key === "salary" && o.category === "salary")
        && !(key === "ns" && o.category === "personal")) continue;
...
function dayMapOf(kassaId, a, b) {
  ...
  // 1b) Usta olgan pul — servis hamyonidan chiqadi
  if (!isCompany) {
    ...
    for (const d of listAllDays(a, b)) {
      ...
      if ((st?.storeId ?? COMPANY) !== kassaId) continue;
      const usd = somToUsd(d.olgan);
      if (!usd) continue;
      const r = row(d.date);
      r.out.service += usd;
