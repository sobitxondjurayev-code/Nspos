...
function turlarniQoy(list) {
  ...
  for (const c of manba) {
    if (c.isActive) EXPENSE_CATEGORIES[c.key] = { label: c.label, group: c.group, service: !!c.service, noteRequired: !!c.noteRequired };
...
const turSync = syncTable("expense_categories", {
  ...
  select: "id,key,label,\"group\",service,note_required,sort,is_active,updated_at",
  realtime: true,
  get: () => kategoriyalar,
  set: (v) => turlarniQoy(v),
  fromRow: (r) => ({
    id: r.id, key: r.key, label: r.label, group: r.group ?? "variable",
    service: !!r.service, noteRequired: !!r.note_required,
    sort: Number(r.sort ?? 100), isActive: r.is_active !== false,
  }),
  toRow: (c) => ({
    key: c.key, label: c.label, group: c.group ?? "variable",
    service: !!c.service, note_required: !!c.noteRequired,
...
export function addExpenseCategory({ label, group = "variable", service = false, noteRequired = false }) {
  const nom = String(label ?? "").trim();
  if (!nom) return null;
  const c = {
    id: "ec-" + Date.now(), key: slug(nom), label: nom, group, service: !!service,
