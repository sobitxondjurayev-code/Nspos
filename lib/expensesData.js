...
export const EXPENSE_CATEGORIES = {
  ...
  delivery: { label: "Dastavka yo'lkira", group: "variable" },
  ...
  other: { label: "Boshqa xarajatlar", group: "variable" },
};
...
export const categoryLabel = (k) =>
  EXPENSE_CATEGORIES[k]?.label ?? LEGACY_CATEGORIES[k] ?? k;
...
// kassada "Boshqa kirim/chiqim" (kassaData.KASSA_CATEGORIES). Bularda
// izoh majburiy — aks holda hisobotda kimga, nimaga ketgani noma'lum
// summa qolib ketadi va tekshirib bo'lmaydi.
const NOTE_REQUIRED = ["other", "other_in", "other_out"];
export const needsNote = (category) => NOTE_REQUIRED.includes(category);
...
export function addRecurring({ category, storeId = "all", amount, amountSom = null, rateUsed = null, method = "cash", kassa = "company", staffId = null, paidTo = null, day = 1, from, to = null, note = "" }) {
  ...
  // vaqtinchalik id o'sha id bilan almashtiriladi.
  insert(TABLE, toRow(r), "xarajat qo'shish").then((res) => {
    if (res.data) recurring = recurring.map((x) => (x.id === r.id ? { ...x, id: res.data.id } : x));
  });
  return r;
}
...
export function updateRecurring(id, patch) {
  ...
  if (next) dbUpdate(TABLE, id, toRow(next), "xarajatni yangilash");
  return next;
}

export function removeRecurring(id) {
  recurring = recurring.filter((r) => r.id !== id);
  dbRemove(TABLE, id, "xarajatni o'chirish");
}
...
export function addExpense({ date, category, storeId = "all", amount, amountSom = null, rateUsed = null, method = "cash", kassa = "company", staffId = null, paidTo = null, note = "" }) {
  ...
  insert(TABLE, toRow(e), "xarajat qo'shish").then((res) => {
    if (res.data) oneOff = oneOff.map((x) => (x.id === e.id ? { ...x, id: res.data.id } : x));
  });
  return e;
}
...
// qolardi (2026-08-13).
function serviceStore(staffById) {
  for (const d of listAllDays()) {
    if (!(Number(d.service) > 0)) continue;
    const st = staffById.get(d.staffId);
    if (st?.storeId) return st.storeId;
  }
  return null;
}
...
function installerPayouts(inR) {
  ...
  const svcStore = serviceStore(byId);
  ...
  for (const d of listAllDays()) {
    ...
    const amount = +(som / rate).toFixed(2);
    ...
    out.push({
      ...
      amount, amountSom: som, rateUsed: rate,
      // Usta puli servis kassasidan beriladi
      method: "service",
      kassa: st.storeId || svcStore || "company",
