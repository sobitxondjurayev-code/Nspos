...
// Baza tasdiqlagan oxirgi holat — rad etilgan yozuv shunga qaytariladi
...
registerModule("nps_records", {
  ...
  restore: (rows) => {
    ...
    for (const r of rows) { byId.set(r.id, r); tasdiq.set(r.id, r); }
    records = [...byId.values()]; persist();
  },
  apply: ({ eventType, new: row, old }) => {
    if (eventType === "DELETE") { records = records.filter((r) => r.id !== old.id); tasdiq.delete(old.id); }
    else { const it = fromRow(row); records = [...records.filter((r) => r.id !== it.id), it]; tasdiq.set(it.id, it); }
    persist();
  },
});
...
// Hamma baho — oy bo'yicha filtrlanmagan. Eskirish tekshiruvi
// (`audit.js` → MANBALAR) shundan o'qiydi: "oxirgi baho qachon
// kiritilgan" savoli oyga bog'liq emas.
export const allNps = () => [...records];
...
// Baza rad etsa (NPS yozish huquqi yo'q, sessiya tugagan) xotira va
// localStorage ORQAGA qaytadi. Ilgari yozuv `silent` edi va lokal
// nusxada qolardi: retention xodimi bahoni "qo'ydi", rahbar
// kompyuterida esa yo'q edi (2026-09-03).
const orqaga = (id) => {
  const oldingi = tasdiq.get(id);
  records = oldingi ? records.map((r) => (r.id === id ? oldingi : r)) : records.filter((r) => r.id !== id);
  persist(); xotiraYangilandi();
};
const saveNpsRow = (it) => {
  if (DEMO_MODE) return Promise.resolve({ ok: true, demo: true });
  return upsert("nps_records", toRow(it), undefined, "NPS baho").then((r) => {
    if (r?.ok) tasdiq.set(it.id, it);
    else if (r && !r.ok) orqaga(it.id);
    return r;
  });
};
...
export function removeNps(id) {
  ...
  if (DEMO_MODE) return;
  dbRemove("nps_records", id, "NPS baho").then((r) => {
    if (r?.ok) tasdiq.delete(id);
    else if (r && !r.ok) orqaga(id);     // o'chirish rad — qator qaytadi
  });
}
